...
- No `git reset` / `git stash` / `git checkout -- <file>` used
  (one accidental `git stash` early on was immediately
  recovered with `git stash pop`; subsequent operations used
  `git restore --staged` for unstage and `mv` to a backup
  directory for untracked-file moves).
