...
impl<T: crate::ecs::runtime::scheduling::component_id::SchedulableComponent> ComponentSet for T {
    fn mask() -> Result<ComponentMask, MaskError> {
...
impl<T: crate::ecs::runtime::scheduling::component_id::SchedulableResource> ResourceSet for T {
    fn mask() -> Result<ResourceMask, MaskError> {
...
macro_rules! impl_component_set_tuple {
    ($($ty:ident),+) => {
        impl<$($ty: crate::ecs::runtime::scheduling::component_id::SchedulableComponent),+> ComponentSet for ($($ty,)+) {
            fn mask() -> Result<ComponentMask, MaskError> {
                let mut mask = ComponentMask::empty();
...
macro_rules! impl_resource_set_tuple {
    ($($ty:ident),+) => {
        impl<$($ty: crate::ecs::runtime::scheduling::component_id::SchedulableResource),+> ResourceSet for ($($ty,)+) {
            fn mask() -> Result<ResourceMask, MaskError> {
                let mut mask = ResourceMask::empty();
