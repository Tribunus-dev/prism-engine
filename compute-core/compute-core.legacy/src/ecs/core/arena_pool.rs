...
use crate::ecs::arena::{Arena, DataType};
use crate::ecs::arena_lifecycle::LifecycleState;
...
mod tests {
    ...
    fn test_pool_reuse() {
        let pool = ArenaPool::new(1024 * 1024, 4);
        let (arena1, was1) = pool.acquire("IOSurfaceFp16ContiguousV1", 1, 4).expect("a1");
        assert!(!was1);
        let id1 = arena1.io_surface_id();
        pool.release("IOSurfaceFp16ContiguousV1", 1, 4, arena1)
            .expect("r1");

        let (arena2, was2) = pool.acquire("IOSurfaceFp16ContiguousV1", 1, 4).expect("a2");
        assert!(was2, "should have reused pooled arena");
        assert_eq!(arena2.io_surface_id(), id1, "same IOSurface ID on reuse");
    ...
    fn test_pool_budget() {
        ...
        let (arena1, _) = pool.acquire("IOSurfaceFp16ContiguousV1", 1, 4).expect("a1");
    ...
    fn test_pool_max_per_key() {
        ...
        let (a1, _) = pool.acquire("IOSurfaceFp16ContiguousV1", 1, 4).expect("a1");
        let (a2, _) = pool.acquire("IOSurfaceFp16ContiguousV1", 1, 4).expect("a2");
        let (a3, _) = pool.acquire("IOSurfaceFp16ContiguousV1", 1, 4).expect("a3");
        ...
        assert!(stats.total_entries <= 2, "pool should cap at max_per_key");
    }
}
