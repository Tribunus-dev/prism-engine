...
pub enum GenerationEvent {
    ...
    Cancelled,
    /// A committed text segment from diffusion — never unresolved canvas tokens.
    CommittedText(String),
    /// Diffusion denoising progress with step position and committed/total counts.
    DiffusionProgress {
        step: u32,
        total_steps: u32,
        committed: u32,
...
fn validate_event_sequence_impl(events: &[GenerationEvent]) -> bool {
    ...
    for (i, event) in events.iter().enumerate() {
        match event {
            ...
            | GenerationEvent::CommittedText(_)
            | GenerationEvent::DiffusionProgress { .. } => {
                if !active {
