...
impl ToolchainAttestation {
    ...
    pub fn probe() -> Result<Self, String> {
        ...
        let coremlcompiler_path = String::from_utf8_lossy(&cmc_path.stdout).trim().to_string();
        ...
        let cmc_ver = Command::new(&coremlcompiler_path)
            ...
            .unwrap_or_else(|e| {
                Command::new("xcrun")
                    ...
                    .output()
                    .unwrap_or_else(|_| panic!("cannot run coremlcompiler --version: {e}"))
            });
        let coremlcompiler_version = String::from_utf8_lossy(&cmc_ver.stdout).trim().to_string();
    ...
    pub fn with_compile_result(
        ...
        self.stdout_sha256 = format!("{:x}", Sha256::digest(&output.stdout));
        self.stderr_sha256 = format!("{:x}", Sha256::digest(&output.stderr));
