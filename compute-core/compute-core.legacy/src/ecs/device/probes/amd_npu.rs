...
use crate::ecs::device::DeviceInfo;
#[cfg(target_os = "linux")]
use crate::ecs::device::{BackendKind, DeviceKind, DeviceMemoryInfo};
    ...
    if let Ok(entries) = std::fs::read_dir("/sys/class/amdxdna/") {
        for entry in entries.flatten() {
            ...
            devices.push(DeviceInfo {
                id: crate::ecs::device::DeviceId(0), // registry reassigns real ids
                kind: DeviceKind::Npu,
        ...
        if let Ok(entries) = std::fs::read_dir("/dev/dri/") {
            for entry in entries.flatten() {
                ...
                if let Ok(uevent) = std::fs::read_to_string(&uevent_path) {
                    ...
                    devices.push(DeviceInfo {
                        id: crate::ecs::device::DeviceId(0),
                        kind: DeviceKind::Npu,
                        backend: BackendKind::Rocm,
                        name: npu_name,
                        vendor: "AMD".to_string(),
