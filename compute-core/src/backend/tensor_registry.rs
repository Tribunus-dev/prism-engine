...
impl MaterializedTensor {
    ...
    pub fn authoritative_backend(&self) -> BackendId {
        self.authoritative_backend
    }
    pub fn version(&self) -> TensorVersion {
        self.version
    }
    ...
    pub fn is_fresh(&self, backend_id: BackendId) -> bool {
        self.materializations
            .get(&backend_id)
            .map(|r| r.state == MaterializationState::Fresh && r.tensor_version == self.version)
            .unwrap_or(false)
    }
    ...
    fn validate_materialization(&self, dtype: DType, shape: &TensorShape) -> Result<(), String> {
        ...
        if shape.dims.len() != self.contract.logical_shape.dims.len() {
            return Err(format!(
                ...
                shape.dims.len(),
                self.contract.logical_shape.dims.len(),
            ));
        }
        Ok(())
    }
    ...
    fn commit_transition(
        ...
        let previous_authoritative_materialization = self
            .materializations
            .get(&self.authoritative_backend)
            .cloned();

        let replaced_destination_materialization = self
            .materializations
            .get(&new_authoritative_backend)
            .cloned();
...
impl TensorRegistry {
    ...
    pub fn add_materialization(
        ...
        let tensor = self
            .tensors
            .get_mut(&tensor_id)
            .ok_or_else(|| format!("TensorRegistry: tensor {:?} not found", tensor_id))?;
