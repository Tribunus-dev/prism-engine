    ...
    for cmd in rx {
        match cmd {
            ...
            InferenceCommand::StartGeneration {
                ...
                // Force MLX to CPU to avoid GPU crashes with the current
                // quantized format (SIGSEGV/SIGBUS in Metal kernel).
                // TODO: remove once quantized format is GPU-compatible.
                mlx_rs::Device::set_default(&mlx_rs::Device::new(mlx_rs::DeviceType::Cpu, 1));
                mlx_rs::Device::set_default(&mlx_rs::Device::new(mlx_rs::DeviceType::Cpu, 0));
                log_info!("[worker {}] MLX device set to CPU", worker_id);
