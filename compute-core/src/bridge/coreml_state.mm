...
int tribunus_coreml_predict_stateful(
    ...
    @try {
        ...
        // Retain pixel buffers for the duration of prediction
        // to prevent use-after-free if the Rust Arena is dropped.
        CVPixelBufferRef inPixBuf = (CVPixelBufferRef)input_arena->cv_buffer;
        CVPixelBufferRef outPixBuf = (CVPixelBufferRef)output_arena->cv_buffer;
        CVPixelBufferRetain(inPixBuf);
        CVPixelBufferRetain(outPixBuf);
