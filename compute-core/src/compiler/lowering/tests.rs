...
use crate::backend::routing::{LogicalShape, OperationFamily, OperationId, Phase, TensorId};
...
fn mlx_preserves_matmul_route() {
    ...
    let receipt = lower_matmul_mlx(&dataset, digest).expect("MLX lowering must succeed");

    assert!(
        receipt.output_verified,
...
fn accelerate_preserves_matmul_route() {
    ...
    let receipt =
        lower_matmul_accelerate(&dataset, digest).expect("Accelerate lowering must succeed");
