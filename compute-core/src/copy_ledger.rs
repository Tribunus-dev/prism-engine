...
pub fn audit_mapped_weight_layout(segments: &[crate::config::PlannedSegment]) -> CopyLedger {
    ...
    for seg in segments {
        ...
        let (storage, contiguous, dtype) = if is_packed {
            (SourceStorage::MappedImage, true, Some("u32→f16".into()))
        } else if is_scale {
            (SourceStorage::MappedImage, true, None)
        } else {
            (SourceStorage::MappedImage, false, None)
        };
...
mod tests {
    ...
    fn test_total_bytes_copied() {
        ...
        let entry_b = CopyEntry::new(
            "b",
            SourceStorage::CopiedVec,
            SourceStorage::MappedImage,
            2000,
            "test",
        );
