...
pub fn hash_directory_deterministic(
    path: &Path,
    exclusions: &[&str],
) -> std::io::Result<DirectoryHashResult> {
...
mod tests {
    ...
    fn subdirectory_order_randomization() {
        ...
        assert_eq!(
            ha.digest, hb.digest,
            "file order should not affect hash with sorted walk"
        );
    ...
    fn exclusion_skips_file() {
        ...
        assert_ne!(
            no_excl.digest, with_excl.digest,
            "excluded files should change hash"
        );
        assert_eq!(
            with_excl.file_count, 2,
            "should only hash 2 files when volatile.bin excluded"
        );
        assert_eq!(
            with_excl.excluded.len(),
            1,
            "should record one excluded path"
        );
    ...
    fn file_len_separator_prevents_collision() {
        ...
        let dir = create_test_dir("collision", &[("f", b"abc")]);
        let result = hash_directory_deterministic(&dir, &[]).unwrap();
