...
pub fn transpose(x: &[f32], src_dims: &[i32], axes: &[i32]) -> Vec<f32> {
    ...
    let out_dims: Vec<usize> = axes
        .iter()
        .map(|&a| src_dims[a as usize] as usize)
        .collect();
...
pub fn evaluate_decode_attention_with_cache(
    ...
    match contract.mutation {
        ...
        KvMutationMode::Append => {
            ...
            // Flatten cache back
            let flat_cache_k: Vec<f32> = new_cache_k
                .iter()
                .flat_map(|head| head.iter().flat_map(|t| t.iter()))
                .copied()
                .collect();
            let flat_cache_v: Vec<f32> = new_cache_v
                .iter()
                .flat_map(|head| head.iter().flat_map(|t| t.iter()))
                .copied()
                .collect();
...
fn compute_attention(
    ...
    k_heads: &[Vec<f32>],
    v_heads: &[Vec<f32>],
    _cached_k: &[Vec<Vec<f32>>],
    _cached_v: &[Vec<Vec<f32>>],
    kv_heads: usize,
    head_dim: usize,
    _cache_len: usize,
    _position: usize,
...
mod tests {
    ...
    fn attention_cacheless() {
        let binding = kv_small_binding();
        let contract =
            binding.to_kv_contract(crate::pipeline_parity::PipelinePhase::KvRead, 0, 0, 0, 0);
        ...
        assert!(
            !output.is_empty(),
            "cacheless attention must produce output"
        );
    ...
    fn attention_cache_read_only() {
        ...
        assert!(
            !output.is_empty(),
            "read-only attention must produce output"
        );
    ...
    fn attention_cache_append() {
        ...
        assert_eq!(
            update_k.as_ref().unwrap().len(),
            expected_elements,
            "appended cache_k should have {} elements",
            expected_elements
        );
    }
