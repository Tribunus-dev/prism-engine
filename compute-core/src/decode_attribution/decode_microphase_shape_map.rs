...
use crate::pipeline_parity::{self, KvCacheLayout, KvCachePhaseContract, KvMutationMode};
...
impl DecodeShapeBinding {
    ...
    pub fn to_kv_contract(
        ...
        let mutation = match phase {
            pipeline_parity::PipelinePhase::KvRead | pipeline_parity::PipelinePhase::KvView => {
                KvMutationMode::ReadOnly
            }
...
pub fn decode_shape_for(semantic_profile_name: &str) -> Option<&'static DecodeShapeBinding> {
    ALL_DECODE_SHAPES
        .iter()
        .find(|p| p.profile_name == semantic_profile_name)
}
...
mod tests {
    ...
    fn decode_small_v1_has_kv_heads() {
        assert_eq!(
            DECODE_SMALL_V1.kv_heads, 2,
            "decode_small_v1 must have kv_heads=2"
        );
    }
    ...
    fn decode_medium_v1_has_kv_heads() {
        assert_eq!(
            DECODE_MEDIUM_V1.kv_heads, 4,
            "decode_medium_v1 must have kv_heads=4"
        );
    }
    ...
    fn to_kv_contract_read_only() {
        ...
        assert_eq!(
            contract.mutation,
            crate::pipeline_parity::KvMutationMode::ReadOnly,
    ...
    fn to_kv_contract_append() {
        let contract = DECODE_MEDIUM_V1.to_kv_contract(
            crate::pipeline_parity::PipelinePhase::KvAppend,
            32,
            32,
            1,
            0,
        ...
        assert_eq!(
            contract.mutation,
            crate::pipeline_parity::KvMutationMode::Append,
    ...
    fn kv_contracts_serialize() {
        ...
        let json =
            serde_json::to_string(&contract).expect("KvCachePhaseContract must serialize to JSON");
        assert!(!json.is_empty(), "serialized JSON must not be empty");
        // Verify it has expected fields
        assert!(
            json.contains("kv_read"),
            "serialized JSON should contain phase id"
        );
        assert!(
            json.contains("batch_seq_head_dim"),
            "serialized JSON should contain layout"
        );
    }
}
