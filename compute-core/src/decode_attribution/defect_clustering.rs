...
fn classify_observation(
    ...
    if obs.backend == "mlx" && obs.status == "numerical_divergence" {
        let abs_err = obs
            .max_absolute_error
            .map_or("N/A".into(), |v| format!("{:.6e}", v));
        let cos_sim = obs
            .cosine_similarity
            .map_or("N/A".into(), |v| format!("{:.6}", v));
        return Classification {
...
fn build_clusters(
    ...
    _receipts: &[DecodeAttributionReceipt],
    ...
    for ((kind, _scid, _backend), indices) in groups.iter() {
        ...
        for obs in &cluster_obs {
            obs_to_cluster_id.insert(
                (
                    obs.semantic_contract_id.clone(),
                    obs.shape_profile.clone(),
                    obs.backend.clone(),
                ),
                cid.clone(),
            );
        ...
        let representative_rows: Vec<String> = cluster_obs
            ...
            .map(|o| {
                format!(
                    "{}/{}/{}/{}",
                    o.backend, o.family, o.shape_profile, o.status
                )
            })
            .collect();

        // Evidence summary
        let evidence_summary = if let Some(first_diag) = cluster_obs.first().map(|o| &o.diagnostic)
        {
        ...
        let recommended_next_gate = match kind {
            ...
            ClusterKind::CrossBackendSemanticMismatch => {
                "FIX-SEMANTIC-CONTRACT-REFERENCE-EVALUATOR"
            }
...
pub fn cluster_defects(
    ...
    for receipt in receipts {
        ...
        } else if canonical_family_name(receipt.graph_family.as_str()) != "identity_passthrough" {
            total_tier1 += 1;
            if passes {
                pass_tier1 += 1;
            } else {
...
pub fn write_observations_json(
    observations: &[DefectObservation],
    path: &Path,
) -> Result<(), String> {
...
pub fn write_correlations_json(
    correlations: &[CrossBackendCorrelation],
    path: &Path,
) -> Result<(), String> {
    let json = serde_json::to_string_pretty(correlations)
        .map_err(|e| format!("serialize correlations: {e}"))?;
    std::fs::write(path, &json).map_err(|e| format!("write {:?}: {e}", path))
}
...
pub fn write_summary_md(
    ...
    sorted_clusters.sort_by_key(|c| {
        (
            severity_order(c.severity),
            -(c.representative_rows.len() as i64),
        )
    });
    ...
    for cluster in &sorted_clusters {
        ...
        if !cluster.blocked_next_phases.is_empty() {
            lines.push(format!(
                "- **Blocks**: {}",
                cluster.blocked_next_phases.join(", ")
            ));
        }
        lines.push(format!(
            "- **Recommended gate**: {}",
        ...
        lines.push(
            "| Row | Backend | Family | Shape | Status | Terminal phase | Max abs err | Cos sim |"
                .into(),
        );
        lines.push(
            "|-----|---------|--------|-------|--------|----------------|-------------|---------|"
        ...
        for row in &cluster.representative_rows {
            let _parts: Vec<&str> = row.splitn(4, '/').collect();
            ...
            let metrics: Option<&DefectObservation> = observations
                .iter()
                .find(|o| o.backend == b && o.family == f && o.shape_profile == s);
            if let Some(m) = metrics {
                let abs_err = m.max_absolute_error.map_or("-".into(), |v| {
                    if v > 1e100 {
                        "∞".into()
                    } else {
                        format!("{:.4e}", v)
                ...
                let cos_sim = m.cosine_similarity.map_or("-".into(), |v| {
                    if v >= 0.9999 {
                        "1.0".into()
                    } else {
                        format!("{:.4}", v)
    ...
    let blockers: Vec<&DefectCluster> = sorted_clusters
        ...
        .filter(|c| {
            matches!(
                c.severity,
                Severity::S1BlocksDecodeMicrophase | Severity::S2BlocksBackendParity
            )
        })
        .copied()
        .collect();
...
pub fn write_next_fix_gates_json(clusters: &[DefectCluster], path: &Path) -> Result<(), String> {
    ...
    for cluster in clusters {
        gates
            .entry(cluster.recommended_next_gate.as_str())
            .or_default()
            .push(cluster);
    }
    let fix_gates: Vec<NextFixGate> = gates
        ...
        .collect();
    let json = serde_json::to_string_pretty(&fix_gates)
        .map_err(|e| format!("serialize fix gates: {e}"))?;
    std::fs::write(path, &json).map_err(|e| format!("write {:?}: {e}", path))
}
...
mod tests {
    ...
    fn kv_cluster_kinds_count_14() {
        ...
        for kind in &kinds {
            ...
            let back: ClusterKind =
                serde_json::from_str(&json).expect("ClusterKind must deserialize");
    ...
    fn kv_cluster_kinds_serde_roundtrip() {
        ...
        for kind in &[
            ...
            let back: ClusterKind =
                serde_json::from_str(&json).expect("KV ClusterKind must deserialize");
...
use super::graph_catalog::canonical_family_name;
