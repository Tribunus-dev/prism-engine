...
fn classification_for_tier1(cluster_kind: &str) -> GapClassification {
    match cluster_kind {
        ...
        | "CrossBackendSemanticMismatch" => GapClassification::NumericalDivergence,
...
fn gap_sort_cmp(
    ...
    (4 - left.severity.as_numeric())
        ...
        .then_with(|| {
            u8::from(!left.blocks_runtime_claim).cmp(&u8::from(!right.blocks_runtime_claim))
        })
        .then_with(|| {
            u8::from(!left.blocks_release_claim).cmp(&u8::from(!right.blocks_release_claim))
        })
        .then_with(|| source_sort_rank(left.source).cmp(&source_sort_rank(right.source)))
        .then_with(|| {
            subsystem_sort_rank(left.subsystem).cmp(&subsystem_sort_rank(right.subsystem))
        })
        .then_with(|| {
            tier_sort_rank(left.tier.as_deref()).cmp(&tier_sort_rank(right.tier.as_deref()))
        })
...
fn gap_matches_external_array_allowlist(name: &str) -> bool {
    KNOWN_EXTERNAL_ARRAY_ALLOWLIST
        .iter()
        .any(|allowed| allowed == &name)
}

fn gap_from_missing_evidence(
    source: GapSource,
    subsystem: GapSubsystem,
    message: String,
...
fn gap_from_tier2_support_row(row: Tier2SupportRow) -> Option<ComputeGap> {
    ...
    Some(ComputeGap {
        ...
        blocks_runtime_claim: if unsupported {
            required
        } else {
            severity_for_support_status(&row.support_status).is_s3_or_higher()
        },
        blocks_release_claim: if unsupported {
            required
        } else {
            severity_for_support_status(&row.support_status).is_s4()
        },
    })
}
...
fn parse_kv_contract_value(contract: &Value, backend: Option<&str>) -> Option<ComputeGap> {
    ...
    let phase = contract
        ...
        .or_else(|| {
            contract_id
                .split('/')
                .next()
                .map(|part| part.trim_start_matches("kv.").to_string())
        });
    ...
    match ownership {
        ...
        "pending_qualification" => Some(ComputeGap {
            ...
            message: if message.is_empty() {
                "pending KV contract qualification".into()
            } else {
                message
            },
            backend,
        ...
        "backend_owned" => {
            ...
            if backend_is_coreml {
                Some(ComputeGap {
                    ...
                    status: GapStatus::NeedsTriage,
                    message: "Core ML backend-owned KV mutation is a false-qualification risk"
                        .into(),
                    backend,
                    phase,
                    family: None,
...
pub fn load_cargo_build_log(path: &Path) -> Result<Vec<ComputeGap>, String> {
    ...
    Ok(text
        ...
        .map(|line| {
            gap_from_warning(
                GapSource::Rustc,
                GapClassification::CompileWarning,
                line.trim().to_string(),
            )
        })
        .collect())
...
pub fn load_cargo_clippy_log(path: &Path) -> Result<Vec<ComputeGap>, String> {
    ...
    Ok(text
        ...
        .map(|line| {
            gap_from_warning(
                GapSource::Clippy,
                GapClassification::ClippyWarning,
                line.trim().to_string(),
            )
        })
        .collect())
    ...
    for line in text.lines() {
        ...
        if let Some(rest) = line.strip_prefix("test ") {
            if let Some(name) = rest.strip_suffix(" FAILED") {
                ...
                let status = if test_name.contains("external_array")
                    && gap_matches_external_array_allowlist(&test_name)
                {
...
fn parse_cargo_test_failure_details(text: &str) -> BTreeMap<String, String> {
    ...
    let flush = |details: &mut BTreeMap<String, String>,
...
pub fn load_tier1_defects(run_dir: &Path) -> Result<Vec<ComputeGap>, String> {
    let cluster_path = run_dir
        .join("clustering")
        .join("tier1_defect_clusters.json");
    ...
    let text =
        fs::read_to_string(&cluster_path).map_err(|e| format!("read {:?}: {e}", cluster_path))?;
...
pub fn load_tier2_manifest(output_dir: &Path) -> Result<Vec<ComputeGap>, String> {
    ...
    if support_path.exists() {
        let text = fs::read_to_string(&support_path)
            .map_err(|e| format!("read {:?}: {e}", support_path))?;
    ...
    if receipt_path.exists() {
        let text = fs::read_to_string(&receipt_path)
            .map_err(|e| format!("read {:?}: {e}", receipt_path))?;
...
pub fn load_support_matrix(output_dir: &Path) -> Result<Vec<ComputeGap>, String> {
    ...
    let claimed_text =
        fs::read_to_string(&claimed_path).map_err(|e| format!("read {:?}: {e}", claimed_path))?;
    let evidence_text =
        fs::read_to_string(&evidence_path).map_err(|e| format!("read {:?}: {e}", evidence_path))?;
    let claimed_rows: Vec<Value> = serde_json::from_str(&claimed_text)
        .map_err(|e| format!("parse {:?}: {e}", claimed_path))?;
    let evidence_rows: Vec<Value> = serde_json::from_str(&evidence_text)
        .map_err(|e| format!("parse {:?}: {e}", evidence_path))?;
    ...
    for row in evidence_rows {
        ...
        let shape_profile = row
            ...
            .unwrap_or_default()
            .to_string();
        let semantic_shape_profile = row
            .get("semantic_shape_profile")
            .and_then(Value::as_str)
            .unwrap_or_default()
            .to_string();
        let row_id = row
    ...
    for row in claimed_rows {
        ...
        let claimed = row
            ...
            .and_then(Value::as_str)
            .unwrap_or_default();
        if let Some(evidence_row) = evidence.get(&(
            backend.clone(),
            family.clone(),
            phase.clone(),
            shape_profile.clone(),
            semantic_shape_profile.clone(),
            row_id.clone(),
        )) {
            let actual = evidence_row
                .get("support_status")
                .and_then(Value::as_str)
                .unwrap_or_default();
            if claimed != actual {
                gaps.push(ComputeGap {
                    ...
                    severity: if actual.starts_with("unsupported") {
                        GapSeverity::S3
                    } else {
                        GapSeverity::S2
                    },
                    ...
                    status: GapStatus::NeedsTriage,
                    message: format!(
                        "support claim '{claimed}' disagrees with evidence '{actual}'"
                    ),
...
fn collect_python_gap(path: &Path, gaps: &mut Vec<ComputeGap>) -> Result<(), String> {
    ...
    let file_name = path
        ...
        .unwrap_or_default()
        .to_lowercase();
    if (file_name.contains("compare")
        || file_name.contains("diff")
        || file_name.contains("reference")
...
pub fn normalize_gaps(
    gaps: &[ComputeGap],
    accounting: GapReportAccounting,
) -> (Vec<ComputeGap>, GapReportReceipt) {
    ...
    let total_warnings = assigned
        ...
        .filter(|gap| {
            matches!(
                gap.classification,
                GapClassification::CompileWarning | GapClassification::ClippyWarning
            )
        })
        .count() as u32;
    let total_errors = assigned
        .iter()
        .filter(|gap| gap.severity.is_s3_or_higher())
        .count() as u32;
    let total_failing_tests = assigned
        .iter()
        .filter(|gap| gap.classification == GapClassification::TestFailure)
        .count() as u32;
    ...
    let tier0_pass_count = assigned
        .iter()
        .filter(|gap| {
            matches!(gap.source, GapSource::SupportMatrix)
                && matches!(gap.phase.as_deref(), Some(phase) if !phase.is_empty())
        })
        .count() as u32;
    ...
    let tier1_nonpass_count = assigned
        ...
        .filter(|gap| matches!(gap.source, GapSource::Tier1DefectCluster) && gap.blocks_promotion)
        .count() as u32;
    let tier2_manifest_count = assigned
        .iter()
        .filter(|gap| matches!(gap.source, GapSource::Tier2Manifest))
        .count() as u32;
    let kv_contract_count = assigned
        .iter()
        .filter(|gap| matches!(gap.source, GapSource::KvContract))
        .count() as u32;
    let false_qualification_risks_detected = assigned.iter().any(|gap| {
        gap.severity.is_s4()
            ...
            && gap
                .phase
                .as_deref()
                .is_some_and(|phase| phase.contains("kv_"))
    });
...
pub fn build_backend_gap_matrix() -> BackendGapMatrix {
    let phases: Vec<String> = PipelinePhase::all()
        .iter()
        .copied()
        .map(phase_str)
        .collect();
    ...
    for phase in PipelinePhase::all().iter().copied() {
        let status_for_backend = |backend: BackendKind| {
            let status = if is_kv_phase(phase) {
                kv_phase_support_for(backend)
                    .get(&phase)
                    .cloned()
                    .unwrap_or(PhaseSupportStatus::Pending {
                ...
                let decoded = match phase {
                    PipelinePhase::QkvProjection => {
                        decode_microphase_support_for("decode_qkv_projection", backend)
                    }
                    PipelinePhase::AttentionOutputProjection => {
                        decode_microphase_support_for("decode_attention_output_projection", backend)
                    }
                    PipelinePhase::ResidualAdd1 => {
                        decode_microphase_support_for("decode_residual_add_1", backend)
                    }
                    PipelinePhase::MlpGateUp => {
                        decode_microphase_support_for("decode_mlp_gate_up_silu", backend)
                    }
                    PipelinePhase::MlpDown => {
                        decode_microphase_support_for("decode_mlp_down", backend)
                    }
                    PipelinePhase::ResidualAdd2 => {
                        decode_microphase_support_for("decode_residual_add_2", backend)
                    }
                    PipelinePhase::LmHead => {
                        decode_microphase_support_for("decode_lm_head", backend)
                    }
                    _ => {
...
fn group_warnings_by_subsystem(gaps: &[ComputeGap]) -> BTreeMap<String, usize> {
    ...
    for gap in gaps.iter().filter(|gap| {
        matches!(
            gap.classification,
            GapClassification::CompileWarning | GapClassification::ClippyWarning
        )
...
fn output_gaps_json(
    path: &Path,
    gaps: &[ComputeGap],
    filter: impl Fn(&ComputeGap) -> bool,
) -> Result<u64, String> {
...
pub fn generate_summary_md(
    ...
    lines.push(format!(
        ...
        gaps.len(),
        receipt.total_warnings,
        receipt.total_errors,
        receipt.total_failing_tests
    ));
    lines.push(format!(
    ...
    lines.push(format!(
        ...
        receipt.observed_tier2_manifest_rows, receipt.tier2_gap_rows
    ));
    lines.push(format!(
        "| KV contracts | {} | {} | n/a | n/a |",
        receipt.observed_kv_contract_rows, receipt.kv_contract_gap_rows
    ));
    lines.push(String::new());

    let critical_blockers: Vec<&ComputeGap> = gaps
        .iter()
        .filter(|gap| gap.severity.is_s3_or_higher())
        .collect();
    ...
    let test_failures: Vec<&ComputeGap> = gaps
        .iter()
        .filter(|gap| gap.classification == GapClassification::TestFailure)
        .collect();
    ...
    let tier1 = gaps
        .iter()
        .filter(|gap| matches!(gap.source, GapSource::Tier1DefectCluster))
        .collect::<Vec<_>>();
    if tier1.is_empty() {
    ...
    for gate in generate_fix_gates(gaps) {
        let severity_rank = gate
            ...
            .max()
            .unwrap_or(0);
        let entry = gate_rows.entry(gate.gate_id.clone()).or_insert((
            gate.priority.clone(),
            BTreeSet::new(),
            severity_rank,
        ));
...
pub fn write_gap_report_artifacts(
    ...
    receipts.insert(
        ...
        ArtifactReceiptEntry {
            sha256: sha256_hex(
                &fs::read(&gap_ledger_path)
                    .map_err(|e| format!("read {:?}: {e}", gap_ledger_path))?,
            ),
    ...
    let warnings_count = output_gaps_json(&warnings_path, gaps, |gap| {
        matches!(
            gap.classification,
            GapClassification::CompileWarning | GapClassification::ClippyWarning
        )
    ...
    receipts.insert(
        ...
        ArtifactReceiptEntry {
            sha256: sha256_hex(
                &fs::read(&warnings_path).map_err(|e| format!("read {:?}: {e}", warnings_path))?,
            ),
    ...
    receipts.insert(
        ...
        ArtifactReceiptEntry {
            sha256: sha256_hex(
                &fs::read(&errors_path).map_err(|e| format!("read {:?}: {e}", errors_path))?,
            ),
    ...
    let test_failures_count = output_gaps_json(&test_failures_path, gaps, |gap| {
        gap.classification == GapClassification::TestFailure
    })?;
    receipts.insert(
        ...
        ArtifactReceiptEntry {
            sha256: sha256_hex(
                &fs::read(&test_failures_path)
                    .map_err(|e| format!("read {:?}: {e}", test_failures_path))?,
            ),
    ...
    let tier1_count = output_gaps_json(&tier1_coverage_path, gaps, |gap| {
        matches!(gap.source, GapSource::Tier1DefectCluster)
    })?;
    receipts.insert(
        ...
        ArtifactReceiptEntry {
            sha256: sha256_hex(
                &fs::read(&tier1_coverage_path)
                    .map_err(|e| format!("read {:?}: {e}", tier1_coverage_path))?,
            ),
    ...
    let tier2_count = output_gaps_json(&tier2_coverage_path, gaps, |gap| {
        matches!(gap.source, GapSource::Tier2Manifest)
    })?;
    receipts.insert(
        "tier2_coverage.json".into(),
        ArtifactReceiptEntry {
            sha256: sha256_hex(
                &fs::read(&tier2_coverage_path)
                    .map_err(|e| format!("read {:?}: {e}", tier2_coverage_path))?,
            ),
    ...
    let kv_count = output_gaps_json(&kv_contract_gaps_path, gaps, |gap| {
        matches!(
            gap.classification,
            GapClassification::KvContractGap | GapClassification::BackendOwnershipGap
        )
    })?;
    receipts.insert(
        ...
        ArtifactReceiptEntry {
            sha256: sha256_hex(
                &fs::read(&kv_contract_gaps_path)
                    .map_err(|e| format!("read {:?}: {e}", kv_contract_gaps_path))?,
            ),
            row_count: kv_count,
        },
    );
    ...
    receipts.insert(
        ...
        ArtifactReceiptEntry {
            sha256: sha256_hex(
                &fs::read(&backend_gap_matrix_path)
                    .map_err(|e| format!("read {:?}: {e}", backend_gap_matrix_path))?,
            ),
    ...
    receipts.insert(
        ...
        ArtifactReceiptEntry {
            sha256: sha256_hex(
                &fs::read(&fix_gates_path)
                    .map_err(|e| format!("read {:?}: {e}", fix_gates_path))?,
            ),
            row_count: fix_gates.len() as u64,
        },
    );

    let blockers: Vec<ComputeGap> = gaps
        .iter()
        .cloned()
        .filter(|gap| gap.blocks_promotion)
        .collect();
    ...
    receipts.insert(
        ...
        ArtifactReceiptEntry {
            sha256: sha256_hex(
                &fs::read(&blockers_path).map_err(|e| format!("read {:?}: {e}", blockers_path))?,
            ),
    ...
    receipts.insert(
        ...
        ArtifactReceiptEntry {
            sha256: sha256_hex(
                &fs::read(&summary_path).map_err(|e| format!("read {:?}: {e}", summary_path))?,
            ),
            row_count: 0,
        },
    );
    ...
    let receipt_index_sha = sha256_hex(
        &fs::read(&receipt_index_path)
            .map_err(|e| format!("read {:?}: {e}", receipt_index_path))?,
    );
    receipts.insert(
...
mod tests {
    ...
    fn gap_sort_priority() {
        ...
        assert_eq!(
            sorted[0].classification,
            GapClassification::BackendOwnershipGap
        );
    }
    ...
    fn external_array_failure_becomes_needs_triage() {
        let gap = gap_from_test_failure(
            "external_array_foo".into(),
            "test external_array_foo ... FAILED".into(),
            GapStatus::NeedsTriage,
        );
    ...
    fn tier1_defect_cluster_to_gap_severity_mapping() {
        assert_eq!(
            severity_for_tier1("S1_blocks_decode_microphase"),
            GapSeverity::S3
        );
    ...
    fn receipt_index_generation() {
        ...
        let receipts = write_gap_report_artifacts(dir.path(), &normalized, &receipt, &matrix)
