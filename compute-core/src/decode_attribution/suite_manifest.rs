    ...
    pub fn new(
        ...
        let (pipeline_phase, phase_variant, semantic_contract_id) = {
            use crate::pipeline_parity::{
                graph_family_phase_variant, graph_family_semantic_contract_id,
                graph_family_to_phase,
            };
            (
        ...
        SuiteRow {
            ...
            semantic_shape_profile: String::new(),
            dtype: dtype.into(),
...
pub fn tier0_manifest() -> SuiteManifest {
    ...
    assert_eq!(
        rows.len(),
        expected_count,
        "Tier 0 expected {expected_count} rows"
    );
    ...
    SuiteManifest {
        ...
        backends: vec![
            ...
            BackendEntry {
                name: "mlx".into(),
                policies: vec!["mlx_default".into()],
            },
            BackendEntry {
                name: "accelerate".into(),
                policies: vec!["accelerate_cpu".into()],
            },
        ],
    }
}
...
pub fn tier1_manifest() -> SuiteManifest {
    ...
    let all_families: Vec<&str> = families_batch1
        .iter()
        .chain(families_batch2.iter())
        .copied()
        .collect();
    let shapes = ["small", "medium"];
    let backends = [
    ...
    assert_eq!(
        rows.len(),
        expected_count,
        "Tier 1 expected {expected_count} rows"
    );
...
pub fn tier2_batch1_manifest() -> SuiteManifest {
    ...
    const BATCH1_FAMILIES: &[(&str, &str, &str, &str)] = &[
        (
            ...
            "qkv_projection",
            "combined_projection",
            "decode.qkv_projection.f32.v1",
        ),
        (
            "decode_attention_output_projection",
            "attention_output_projection",
            "default",
            "decode.attention_output_projection.f32.v1",
        ),
        (
            "decode_residual_add_1",
            "residual_add_1",
            "default",
            "decode.residual_add_1.f32.v1",
        ),
        (
            "decode_mlp_gate_up_silu",
            "mlp_gate_up",
            "gated_silu",
            "decode.mlp_gate_up.silu.f32.v1",
        ),
        (
            "decode_mlp_down",
            "mlp_down",
            "default",
            "decode.mlp_down.f32.v1",
        ),
        (
            "decode_residual_add_2",
            "residual_add_2",
            "default",
            "decode.residual_add_2.f32.v1",
        ),
        (
            "decode_lm_head",
            "lm_head",
            "default",
            "decode.lm_head.f32.v1",
        ),
    ];
    ...
    assert_eq!(
        rows.len(),
        expected,
        "Tier 2 Batch 1 expected {expected} rows"
    );
...
mod tests {
    ...
    fn row_ids_are_deterministic() {
        ...
        assert_eq!(
            ids, ids2,
            "row IDs must be deterministic across invocations"
        );
    }
    ...
    fn row_id_format() {
        ...
        for row in &m.rows {
            assert!(
                row.row_id.starts_with("tier0/"),
                "row_id must start with tier: {}",
                row.row_id
            );
    ...
    fn tier1_has_48_rows() {
        ...
        assert_eq!(
            m.rows.len(),
            48,
            "Tier 1 must have 48 rows (8 families × 2 shapes × 3 backends)"
        );
    }
    ...
    fn tier2_batch1_has_42_rows() {
        ...
        assert_eq!(
            m.rows.len(),
            42,
            "Tier 2 Batch 1 must have 42 rows (7 families × 2 shapes × 3 backends)"
        );
    }
    ...
    fn tier2_batch1_rows_have_phase_metadata() {
        ...
        for row in &m.rows {
            assert!(
                row.pipeline_phase.is_some(),
                "row {} must have pipeline_phase",
                row.row_id
            );
            assert!(
                !row.phase_variant.is_empty(),
                "row {} must have phase_variant",
                row.row_id
            );
            assert!(
                !row.semantic_contract_id.is_empty(),
                "row {} must have semantic_contract_id",
                row.row_id
            );
            assert!(
                !row.semantic_shape_profile.is_empty(),
                "row {} must have semantic_shape_profile",
                row.row_id
            );
            assert_eq!(
                row.tier,
    ...
    fn tier2_manifest_still_42_rows() {
        ...
        assert_eq!(
            m.rows.len(),
            42,
            "Tier 2 Batch 1 manifest must still have 42 rows after KV phase addition"
        );
    }
}
