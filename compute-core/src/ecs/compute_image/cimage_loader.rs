...
pub struct CimageDeployment {
    ...
    pub header: crate::ecs::compute_image::manifest::CImageHeader,
...
impl CimageDeployment {
    ...
    fn load_v1(bytes: Vec<u8>, device: &metal::Device) -> Result<Self, String> {
        ...
        type ManHeader = crate::ecs::compute_image::manifest::CImageHeader;
        let header: ManHeader =
            unsafe { std::ptr::read_unaligned(bytes.as_ptr() as *const ManHeader) };

        if header.magic != crate::ecs::compute_image::manifest::CIMAGE_MAGIC {
            return Err(format!(
                "bad magic: 0x{:08X} (expected 0x{:08X})",
                header.magic,
                crate::ecs::compute_image::manifest::CIMAGE_MAGIC
            ));
        }
    ...
    fn load_v2(bytes: Vec<u8>, device: &metal::Device) -> Result<Self, String> {
        ...
        Ok(Self {
            header: crate::ecs::compute_image::manifest::CImageHeader::default(),
    ...
    pub fn verify(
        ...
        (
            crate::ecs::compute_image::manifest::CImageHeader,
            V1CImageLayoutMeta,
        ),
        String,
    > {
        ...
        type VfyHeader = crate::ecs::compute_image::manifest::CImageHeader;
