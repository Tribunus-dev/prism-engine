...
impl ImageRuntime {
    ...
    fn lookup_handle(&self, lease_handles: &[u64], name: &str) -> Option<Array> {
        ...
        if let Some(&h) = self.persistent_handles.get(name) {
            let reg = crate::ecs::bridge::ARRAY_REGISTRY.read();
    ...
    pub fn run_six_layer_prefix(&mut self) -> crate::Result<Array> {
        ...
        for layer in 0..layer_count {
            ...
            let handles_after = crate::ecs::bridge::handle_count();
            ...
            let handles_retired = crate::ecs::bridge::handle_count();
    ...
    pub fn run_full_model(&mut self, token_ids: &[i32]) -> crate::Result<u32> {
        ...
        let mut caches: Vec<crate::ecs::kv_cache::KvCache> = Vec::with_capacity(plan.layers.len());
        for layer_plan in &plan.layers {
            ...
            caches.push(crate::ecs::kv_cache::KvCache::new(
                capacity, n_kv_heads, head_dim, is_sliding,
            ));
        }

        let idle_handles = crate::ecs::bridge::handle_count();
        eprintln!(
        ...
        let final_handles = crate::ecs::bridge::handle_count();
        eprintln!(
    ...
    pub fn release(&mut self) {
        ...
        for handle in self
            ...
            .collect::<Vec<_>>()
        {
            let _ = crate::ecs::bridge::free_array(handle);
        }
