// ── Fused Down Projection + Residual ───────────────────────────────────────
//
// Fuses down-projection ternary page-640 GEMV with residual addition.
// The input is the MLP activation (gate*SiLU(up) from fused_gate_up_activation
// or equivalent). The residual is the pre-MLP activation (typically same as
// the input to fused_gate_up_activation).
//
//   out[gid] = Σ(W_down[gid][col] * mlp_activation[col]) + residual[gid]
//
// One threadgroup (64 threads) per output element. Down dimension: hidden_dim
// (the transformer's hidden size), input dimension: intermediate_dim (MLP
// expansion factor × hidden_dim).
//
// W_down is a ternary-page640 packed weight matrix.
//
// 3 personalities via mode_flags:
//   deployment: just compute output
//   diagnostic: write AttentionProbe records for sampled output dims
//   fused_scoring: write ErrorPartial for pre-residual vs post-residual
//
// Buffer binding:
//   0 = down_pages       PackedTernaryPage640* — down weight pages [hidden_dim * nt]
//   1 = mlp_input        half*                 — MLP activation [intermediate_dim]
//   2 = residual         half*                 — pre-MLP residual [hidden_dim]
//   3 = page_scales      half*                 — per-page scale [hidden_dim * nt]
//   4 = channel_scales   half*                 — per-weight-position scales
//   5 = sidecar          half*                 — sidecar entries (interleaved)
//   6 = sidecar_offsets  uint*                 — per-page sidecar byte base
//   7 = output           half*                 — down-projection output [hidden_dim]
//   8 = params           ProjectionParams (constant)
//       → in_dim  = intermediate_dim (MLP activation dimension)
//       → out_dim = hidden_dim (output dimension)
//       → page_count = total down projection pages
//       → page_width = default PAGE_WIDTH (640)
//   9 = receipt          KernelReceipt         — instrumentation (optional)
//  10 = probe_output     AttentionProbe*       — diagnostic probe output (optional)
//  11 = error_partials   ErrorPartial*         — fused_scoring output (optional)
//
// Function constant:
...
struct ProjectionParams {
    uint  in_dim;         // intermediate_dim
    uint  out_dim;        // hidden_dim
    uint  page_count;     // total down pages
    uint  page_width;     // default PAGE_WIDTH (640)
    uint  mode_flags;
    uint  probe_seed;
    uint  reserved[5];
};
...
kernel void fused_down_proj_residual(
    device const PackedTernaryPage640* down_pages    [[buffer(0)]],
    device const half*                 mlp_input     [[buffer(1)]],
    ...
    device AttentionProbe*             probe_output  [[buffer(10)]],
    ...
    uint simd_id                                     [[simdgroup_index_in_threadgroup]])
{
    ...
    const uint  intermed    = params.in_dim;
    const uint  hidden_dim  = params.out_dim;
    const uint  page_width  = PAGE_WIDTH_FC > 0 ? PAGE_WIDTH_FC : params.page_width;
    const uint  nt          = (intermed + page_width - 1) / page_width;
    const uint  words_per_page = 32u;
    const uint  weights_per_page = page_width;

    device const PackedTernaryPage640* row_pages = down_pages + (gid * nt);
    ...
    if ((flags & MODE_SIDECAR) && nt > 0) {
        ...
        for (uint p = tid; p < nt; p += 64) {
            ...
            for (uint ei = 0; ei < sc_count; ++ei) {
                ...
                if (col >= intermed) break;
                acc = fma(float(mlp_input[col]), float(sc_vals[ei]) * sc_scale, acc);
            }
    ...
    if (tid == 0) {
        ...
        if ((flags & MODE_DIAGNOSTIC) && probe_output) {
            ...
            if (hash % 32u == 0u) {
                device AttentionProbe& out = probe_output[gid];
                out.head_id            = gid;
                out.token_index        = 0;
        ...
        if ((flags & MODE_RECEIPT) && gid == 0) {
            ...
            receipt->threadgroups           = hidden_dim;
            receipt->threads_per_threadgroup = 64;
            receipt->output_elements        = hidden_dim;
            receipt->flags                  = flags;
            ...
            receipt->logical_activation_bytes = ulong(hidden_dim) * ulong(sizeof(half));
        }
    }
}
