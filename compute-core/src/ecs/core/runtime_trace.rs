...
use crate::ecs::receipts::CopyClassification;
...
impl ExecutionTrace {
    ...
    pub fn quantized_matmul_count(&self) -> usize {
        self.entries
            .iter()
            .filter(|e| e.operation == "quantized_matmul")
            .count()
    }
...
mod tests {
    ...
    fn test_timeline_event_type_as_str() {
        ...
        assert_eq!(
            TimelineEventType::CopyHostToDevice.as_str(),
            "copy_host_to_device"
        );
        assert_eq!(
            TimelineEventType::CopyDeviceToHost.as_str(),
            "copy_device_to_host"
        );
        assert_eq!(TimelineEventType::SyncBarrier.as_str(), "sync_barrier");
        assert_eq!(
            TimelineEventType::TempAllocation.as_str(),
            "temp_allocation"
        );
    ...
    fn test_runtime_timeline_basics() {
        ...
        tl.push_event(TimelineEvent::new(
            100,
            TimelineEventType::EvalStart,
            "first prefill",
        ));
