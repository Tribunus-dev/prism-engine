...
pub struct Gemma4Teacher {
    orch: crate::ecs::legacy_compute_image_core::orchestrator::Orchestrator,
}
...
impl Gemma4Teacher {
    ...
    pub fn load(cimage: impl AsRef<std::path::Path>) -> Result<Self, String> {
        let orch = crate::ecs::legacy_compute_image_core::orchestrator::Orchestrator::from_cimage(
            cimage.as_ref(),
            1,
            false,
        )?;
    ...
    pub fn orchestrator_mut(
        &mut self,
    ) -> &mut crate::ecs::legacy_compute_image_core::orchestrator::Orchestrator {
        &mut self.orch
    }
}
