...
use super::super::compile::execution_graph::{
...
use super::super::compile::source::LoadedSource;
use super::super::compile::source::{source_tensor_byte_len, source_tensor_view};
use super::super::compile::ternary::model_artifact_tag;
use super::super::compile::ternary::{
...
use super::super::compile::tts_compile::pack_tts_weights;
use super::super::manifest::Manifest;
use super::super::manifest::SharedWeightLayout;
use super::super::multimodal::descriptor::{
...
fn stream_ternary_segment_to_mmap_gpu(
    ...
    use super::super::compile::try_ternary_tile640_pack_gpu;
    ...
    for binding_name in weight_names {
        let (out_dim, in_dim) = {
            ...
            for mmap in &loaded.mmap_bytes {
                crate::ecs::legacy_compute_image_core::compile::source::ensure_tensor_loaded(entry, mmap);
...
pub(crate) fn compile_and_pack_god_binary(
    ...
    if matches!(qmode, CompileQuantMode::Nf4Tile640 { .. }) {
        crate::ecs::legacy_compute_image_core::compile::apply_quantize_to_loaded(loaded, qmode)
    ...
    if matches!(qmode, CompileQuantMode::Nf4Tile640 { .. }) {
        ...
        if !embed_key.is_empty() {
            ...
            let raw_bytes = {
                ...
                for mmap in &loaded.mmap_bytes {
                    crate::ecs::legacy_compute_image_core::compile::source::ensure_tensor_loaded(st, mmap);
                    if !st.data.is_empty() {
                        break;
                    }
                }
            ...
            if !raw_bytes.is_empty() {
                crate::ecs::legacy_compute_image_core::compile::try_nf4_tile640_pack_gpu_to_output(
                    loaded,
                    embed_key,
                    &raw_bytes,
                    &vocab_dtype,
                    vocab_out_dim,
                    vocab_in_dim,
                    Some(crate::ecs::legacy_compute_image_core::compile::Nf4Tile640MmapOutput {
        ...
        let gpu_done = {
            ...
            let result = crate::ecs::legacy_compute_image_core::compile::try_ternary_tile640_pack_gpu(
                loaded,
                embed_key,
                &raw_bytes,
                vocab_out_dim,
            ...
            let f32_vals: Vec<f32> = raw_bytes
                ...
                .map(|c| {
                    ...
                    if st.dtype == "BF16" {
                        ...
                        crate::ecs::legacy_compute_image_core::compile::half_to_f32(bits)
                    }
                })
                .collect();
...
fn synthesize_multimodal_segments(
    ...
    let mut image_records = Vec::new();
    let mut audio_records = Vec::new();
    let tensor_by_id: HashMap<u32, &crate::ecs::legacy_compute_image_core::manifest::TensorEntry> = manifest
        .tensor_table
        .iter()
