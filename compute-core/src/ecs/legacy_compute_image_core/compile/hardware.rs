...
fn bench_rms_norm_mlx(candidate: &KernelCandidate, m: u32, k: u32) -> KernelBenchResult {
    use crate::ecs::backend::{MlxBackend, RmsNormOp, TensorBackend};
...
pub fn run_hardware_assessment() -> AssessmentReceipt {
    ...
    if let Some(coreai_result) =
        crate::ecs::backend::coreai_lane::CoreAiLane::new().bench_minimal_subgraph()
    {
    ...
    let mut concurrency = crate::ecs::legacy_compute_image_core::hw_assessment::ConcurrencyPlan::new();
