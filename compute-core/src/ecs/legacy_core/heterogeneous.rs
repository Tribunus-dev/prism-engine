...
pub fn dispatch_rms_norm(
    ...
    if let Some(island) = island {
        ...
        for b in 0..batch {
            ...
            unsafe {
                crate::ecs::backend::accelerate_ffi::vDSP_vmul(
                    x_row.as_ptr(),
                    1,
                    x_row.as_ptr(),
                    1,
            ...
            let mut sum = 0.0f32;
            unsafe {
                crate::ecs::backend::accelerate_ffi::vDSP_sve(x_sq.as_ptr(), 1, &mut sum, dim_i32);
            }
            let inv_rms = 1.0 / ((sum / dim as f32) + eps).sqrt();
            let scalar = [inv_rms];
            unsafe {
                crate::ecs::backend::accelerate_ffi::vDSP_vsmul(
                    x_row.as_ptr(),
                    1,
                    scalar.as_ptr(),
                    out_row.as_mut_ptr(),
...
pub fn dispatch_add(
    ...
    if let Some(island) = island {
        ...
        let ptr = unsafe { arena.base_ptr() as *mut f32 };
        unsafe {
            crate::ecs::backend::accelerate_ffi::vDSP_vadd(
                a_data.as_ptr(),
                1,
                b_data.as_ptr(),
                1,
...
pub fn run_layer_heterogeneous(
    ...
    rope_cos: &Array,
    rope_sin: &Array,
    cache: &mut crate::ecs::kv_cache::KvCache,
    kv_offset: u32,
    rms_norm_eps: f32,
