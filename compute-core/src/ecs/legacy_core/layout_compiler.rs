...
use crate::ecs::mlx_inventory::MlxInventory;
...
fn dtype_info(dtype: &str) -> DtypeInfo {
    match dtype {
        "float32" | "float" => DtypeInfo {
            nbytes: 4,
            alignment: 4,
        },
        "bfloat16" | "float16" | "half" => DtypeInfo {
            nbytes: 2,
            alignment: 2,
        },
        "int8" | "uint8" | "int4" | "uint4" => DtypeInfo {
            nbytes: 1,
            alignment: 1,
        },
        "int32" | "uint32" => DtypeInfo {
            nbytes: 4,
            alignment: 4,
        },
        "int64" | "uint64" => DtypeInfo {
            nbytes: 8,
            alignment: 8,
        },
        _ => DtypeInfo {
...
pub fn compile_layouts(ir: &LayoutIr, _inventory: &MlxInventory) -> Vec<LayoutReport> {
    ...
    activations.sort_by(|a, b| {
        let a_use = ir.ops.iter().position(|op| {
            op.input_tensors.contains(&a.name) || op.output_tensors.contains(&a.name)
        });
        let b_use = ir.ops.iter().position(|op| {
            op.input_tensors.contains(&b.name) || op.output_tensors.contains(&b.name)
        });
        a_use
            .unwrap_or(usize::MAX)
            .cmp(&b_use.unwrap_or(usize::MAX))
    });
...
mod tests {
    ...
    fn compile_layouts_populates_all_tensors() {
        ...
        let inventory = crate::ecs::mlx_inventory::FORK_INVENTORY;
    ...
    fn weight_layout_is_contiguous() {
        ...
        for r in &reports {
            ...
            if t.is_weight {
                ...
                assert_eq!(
                    r.byte_offset, r.aligned_offset,
    ...
    fn validate_layout_rejects_misaligned_offset() {
        ...
        assert!(
            result.is_err(),
            "expected validation error for misaligned offset"
        );
    ...
    fn zero_copy_violation_on_misaligned_weight() {
        ...
        assert!(
            violation.is_some(),
            "expected violation for misaligned offset"
        );
        assert!(
