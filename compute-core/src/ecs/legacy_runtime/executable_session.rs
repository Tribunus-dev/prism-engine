...
pub struct RuntimeBackends {
    ...
    pub accelerate_state: crate::ecs::backend::accelerate_lane::AccelerateLane,
    /// Core ML lane state (ANE subgraph execution).
    pub coreai_state: crate::ecs::backend::coreai_lane::CoreAiLane,
...
impl PreparedExecutableSession {
    ...
    pub fn execute_phase_dag(&self) -> Vec<crate::ecs::scheduling::receipts::PhaseReceipt> {
        let dag = match &self.image.phase_dag {
            Some(d) => d,
            None => return Vec::new(),
        };
        let engine = crate::ecs::scheduling::phase_engine::PhaseEngine::new();
        let mut ctx = crate::ecs::scheduling::execution_context::ExecutionContext::new_empty();
