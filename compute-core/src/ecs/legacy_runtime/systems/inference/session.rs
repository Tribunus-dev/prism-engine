//! Inference session and multi-modal input types — per-request state, memory
//! management, streaming config, and cold-start wrappers.
//!
...
impl WorkingSetManager {
    ...
    pub fn learn_evolk_budgets(
        ...
        cache: &mut crate::ecs::kv_cache::CompressedKvCache,
...
impl ProfiledInferenceSession {
    ...
    pub fn capture_kv_snapshot(&self) -> Vec<crate::ecs::kv_cache::CompressedKvSlot> {
        self.kv_caches
            .iter()
            .map(|kvc| crate::ecs::kv_cache::CompressedKvSlot {
    ...
    pub fn restore_from_kv_snapshot(
        &mut self,
        snapshot: &[crate::ecs::kv_cache::CompressedKvSlot],
    ...
    fn execute_dag_with_kv_guard(
        ...
        let mut ctx = ExecutionContext {
            ...
            backend: Some(Box::new(RuntimeBackends {
                ...
                accelerate_state: crate::ecs::backend::accelerate_lane::AccelerateLane::new(),
                coreai_state: crate::ecs::backend::coreai_lane::CoreAiLane::new(),
        ...
        *guard.caches_mut() = ctx.kv_caches;
        ...
        if result.all_completed {
            ...
            self.kv_caches = guard
                ...
                .into_iter()
                .map(|lc| match lc {
                    crate::ecs::kv_cache::LiveKvCache::Fp16(kv) => kv,
                    _ => {
            ...
            .map(|caches| {
                caches
                    ...
                    .any(|(lc, &ckpt)| match lc {
                        crate::ecs::kv_cache::LiveKvCache::Fp16(c) => c.committed_len != ckpt,
    ...
    pub fn decode_one(
        ...
        if !phase_engine_completed {
            for (l, layer_plan) in plan.layers.iter().enumerate() {
                ...
                let handles_before = crate::ecs::bridge::handle_count();
                let lw = match &mut self.working_set {
                    Some(ws) => ws
                        .weight_streamer
                        .activate(l as u32)
...
pub fn execute_profiled_cold_once(
    ...
    let receipt = ProfiledReceipt {
        ...
        handle_final: crate::ecs::bridge::handle_count() as u64,
        layer_records: plan
            .layers
            .iter()
...
mod tests {
    ...
    fn test_guard_caches_mut_returns_live_ref() {
        ...
        let mut guard = KvCacheRestoreGuard::new(&mut session_caches);
        {
            ...
            if let crate::ecs::kv_cache::LiveKvCache::Fp16(kv) = &mut live[0] {
                kv.committed_len = 10;
            }
        }
        let recovered = guard.into_inner();
        if let crate::ecs::kv_cache::LiveKvCache::Fp16(kv) = &recovered[0] {
...
struct KvCacheRestoreGuard {
    ...
    caches: Option<Vec<crate::ecs::kv_cache::LiveKvCache>>,
}
...
impl KvCacheRestoreGuard {
    ...
    fn new(slot: &mut Vec<KvCache>) -> Self {
        ...
        let caches: Vec<crate::ecs::kv_cache::LiveKvCache> = raw
            .into_iter()
            .map(|c| crate::ecs::kv_cache::LiveKvCache::Fp16(c))
            .collect();
        Self {
    ...
    fn caches_mut(&mut self) -> &mut Vec<crate::ecs::kv_cache::LiveKvCache> {
        self.caches
            .as_mut()
    ...
    fn into_inner(mut self) -> Vec<crate::ecs::kv_cache::LiveKvCache> {
        self.caches
            .take()
            .expect("KvCacheRestoreGuard already disarmed")
    }
