...
impl BackendCapabilityRegistry {
    ...
    pub fn evaluate(
        ...
        let semantics = match group.derive_semantics() {
            Ok(s) => s,
            Err(e) => {
                return some(UnsupportedFusionReason::UnsupportedOp(format!(
                    "semantic error: {e:?}"
                )))
            }
        };
        ...
        if semantics.mixed_codec {
            ...
            if !mp.supports_mixed_precision {
                return some(UnsupportedFusionReason::UnsupportedOp(format!(
                    "mixed precision not supported by {target:?}"
                )));
            }
            ...
            if !mp.supported_scopes.contains(&plan.scope) {
                return some(UnsupportedFusionReason::UnsupportedOp(format!(
                    "precision scope {:?} not supported by {target:?}",
                    plan.scope
                )));
            }
            ...
            if !mp.supported_base_codecs.contains(&plan.default_codec) {
                return some(UnsupportedFusionReason::UnsupportedCodec(
                    plan.default_codec,
                ));
            }
            ...
            for override_entry in &plan.overrides {
                ...
                if let Some(max_frac) = mp.max_override_fraction {
                    ...
                    if override_fraction > max_frac {
                        return some(UnsupportedFusionReason::UnsupportedOp(format!(
                            "override fraction {override_fraction} exceeds max {max_frac}"
                        )));
                    }
                }
            }
        ...
        let group_kinds: Vec<DataflowOpKind> = group.body.iter().map(|n| n.op.kind()).collect();
    ...
    /// Simplified support check — checks whether any registered backend supports
    /// a group with the given `op_kinds` sequence and `role`, without requiring
    /// a full `FusedGroup`.
    ///
    /// Unlike `evaluate()`, this method does NOT check codec support (no
    /// LoadWeight nodes are available in the string-only context). It checks:
    /// 1. Whether the target is registered
    /// 2. Whether the role is supported
    /// 3. Whether the op count is within `max_ops_per_group`
    /// 4. Whether any fusion rule pattern matches the op sequence
    ///
    /// Returns `(bool, Option<UnsupportedFusionReason>)` — `true` if the
    /// target supports the sequence, `false` with a reason otherwise.
    pub fn supports_sequence(
        &self,
        target: BackendLoweringTarget,
        op_kinds: &[DataflowOpKind],
        role: BackendRole,
    ) -> (bool, Option<UnsupportedFusionReason>) {
        let cap = match self.entries.get(&target) {
            Some(c) => c,
            None => {
                return (
                    false,
                    Some(UnsupportedFusionReason::UnsupportedOp(
                        "backend not registered".into(),
                    )),
                )
            }
        ...
        // Check max ops.
        if op_kinds.len() > cap.max_ops_per_group {
            return (
                false,
                Some(UnsupportedFusionReason::ExceedsMaxOps(op_kinds.len())),
            );
        }

        // Check pattern matching against fusion rules.
        // If the backend has no rules, only single-op groups are accepted.
        if cap.rules.is_empty() && op_kinds.len() > 1 {
            return (false, Some(UnsupportedFusionReason::NoRuleMatched));
        }
        let pattern_matched = cap.rules.iter().any(|rule| {
            rule.pattern.len() == op_kinds.len()
                && rule
                    .pattern
                    .iter()
                    .zip(op_kinds.iter())
        ...
        if !pattern_matched && !op_kinds.is_empty() {
            return (false, Some(UnsupportedFusionReason::NoRuleMatched));
        }
    ...
    reg.register(BackendCapability {
        ...
        mixed_precision: MixedPrecisionCapability {
            ...
            supported_scopes: vec![
                PrecisionScope::Tile,
                PrecisionScope::Group,
                PrecisionScope::InputAxisSlice,
                PrecisionScope::OutputAxisSlice,
            ],
...
mod tests {
    ...
    use crate::ecs::plan::fusion::{DataflowNode, DataflowOp};
    use crate::ecs::plan::CodecFamily;
    use BackendLoweringTarget::*;
    use BackendRole::*;
    ...
    fn ane_rejects_nf4_with_unsupported_codec() {
        ...
        assert!(
            matches!(
                result.reason,
                Some(UnsupportedFusionReason::UnsupportedCodec(CodecFamily::Nf4))
            ),
    ...
    fn ane_rejects_sym_int4_with_unsupported_codec() {
        ...
        assert!(
            matches!(
                result.reason,
                Some(UnsupportedFusionReason::UnsupportedCodec(
                    CodecFamily::SymInt4
                ))
            ),
    ...
    fn metal_rejects_exceeds_max_ops() {
        ...
        assert!(
            matches!(
                result.reason,
                Some(UnsupportedFusionReason::ExceedsMaxOps(5))
            ),
    ...
    fn core_ml_no_fusion() {
        ...
        assert!(
            matches!(
                result.reason,
                Some(UnsupportedFusionReason::ExceedsMaxOps(2))
            ),
    ...
    fn supports_method_correctness_all_backends() {
        ...
        let required: std::collections::HashSet<BackendLoweringTarget> = vec![
            MetalFusedGpu,
            AnePlanarEngine,
            CoreMlHighLevel,
            AccelerateRayonCpu,
