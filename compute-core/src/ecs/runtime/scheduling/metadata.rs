...
impl<'a> Resources<'a> {
    ...
    pub fn get<T: crate::ecs::runtime::world::Resource>(&self) -> Option<&T> {
        self.0.get_resource::<T>()
    }

    /// Mutate a resource in the World.
    pub fn get_mut<T: crate::ecs::runtime::world::Resource>(&mut self) -> Option<&mut T> {
        self.0.get_resource_mut::<T>()
    }

    /// Insert or replace a resource.
    pub fn insert<T: crate::ecs::runtime::world::Resource>(&mut self, resource: T) {
