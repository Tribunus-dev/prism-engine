...
impl World {
    ...
    pub fn entity_count(&self) -> usize {
        self.components
            .values()
            .filter_map(|s| downcast_component_vec_len(s as &dyn std::any::Any))
            .next()
            .unwrap_or(0)
    }
    ...
    pub fn total_component_count(&self) -> usize {
        self.components
            .values()
            .map(|s| component_vec_len(s as &dyn std::any::Any))
            .sum()
    }
...
fn downcast_component_vec_len(storage: &dyn std::any::Any) -> Option<usize> {
    storage
        .downcast_ref::<ComponentVec<super::agent_slot::AgentSlot>>()
        .map(|v| v.len())
}

fn component_vec_len(storage: &dyn std::any::Any) -> usize {
    if let Some(v) = storage.downcast_ref::<ComponentVec<super::agent_slot::AgentSlot>>() {
        v.len()
    } else if let Some(v) = storage.downcast_ref::<ComponentVec<super::components::KVCacheRef>>() {
        v.len()
    } else if let Some(v) = storage.downcast_ref::<ComponentVec<super::components::AgentPayload>>()
    {
        v.len()
    } else if let Some(v) = storage.downcast_ref::<ComponentVec<super::components::ToolRegistry>>()
    {
        v.len()
    } else {
        0
...
fn remove_component_from_any(storage: &mut dyn std::any::Any, entity: Entity) {
    ...
    if let Some(s) = storage.downcast_mut::<ComponentVec<super::agent_slot::AgentSlot>>() {
        s.remove(entity);
    } else if let Some(s) = storage.downcast_mut::<ComponentVec<super::components::KVCacheRef>>() {
        s.remove(entity);
    } else if let Some(s) = storage.downcast_mut::<ComponentVec<super::components::AgentPayload>>()
    {
        s.remove(entity);
    } else if let Some(s) = storage.downcast_mut::<ComponentVec<super::components::ToolRegistry>>()
    {
        s.remove(entity);
    }
}
