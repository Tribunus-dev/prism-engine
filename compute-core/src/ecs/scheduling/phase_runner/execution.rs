//! Core phase execution — [`PhaseRunner`] trait and all concrete runner
//! implementations.
//!
//! Each concrete runner maps a [`PhaseKind`] to its actual work: MLX decode,
//! Metal kernel dispatch, Core ML graph execution, Accelerate SIMD ops, etc.
//!
//! New runners should implement [`PhaseRunner`] and register themselves in
//! [`PhaseRunnerRegistry::new`](super::dispatch::PhaseRunnerRegistry::new).
...
impl PhaseRunner for MetalFusedKernelRunner {
    ...
    fn run(&self, phase: &EmittedPhase, ctx: &mut ExecutionContext) -> Result<(), String> {
        ...
        // Admission gate.
        use crate::ecs::benchmark::admission::{
...
impl PhaseRunner for LegacyMlxLayerRunner {
    ...
    fn run(&self, phase: &EmittedPhase, ctx: &mut ExecutionContext) -> Result<(), String> {
        ...
        let kv_cache = match ctx.kv_caches.get_mut(layer_idx) {
            Some(crate::ecs::kv_cache::LiveKvCache::Fp16(ref mut kv)) => kv,
            Some(_) => {
                return Err(format!(
                    "LegacyMlxLayer layer {}: LiveKvCache variant is not Fp16 (compression not wired yet)",
                    layer_idx
                ))
            }
            None => {
                return Err(format!(
                    "LegacyMlxLayer: layer index {} out of bounds for kv_caches (len={})",
                    layer_idx,
                    ctx.kv_caches.len()
                ))
            }
...
impl PhaseRunner for LegacyMlxPrologueRunner {
    ...
    fn run(&self, phase: &EmittedPhase, ctx: &mut ExecutionContext) -> Result<(), String> {
        ...
        let plan = crate::ecs::config::ProloguePlan {
