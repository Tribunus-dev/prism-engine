...
use crate::ecs::compute_image::phase_graph::{ArtifactBindingId, PhaseId, WeightResidencySetId};
    ...
    fn from(receipts: &[PhaseReceipt]) -> Self {
        ...
        let fallback_count = receipts
            ...
            .filter(|r| {
                matches!(
                    r.status,
                    crate::ecs::compute_image::phase_dag::PhaseCompletionStatus::FallbackUsed(_)
                )
            })
            .count();
