...
use crate::ecs::server::distill_worker::{
...
pub fn create_router(state: AppState) -> Router {
    ...
    Router::new()
        ...
        .merge(crate::ecs::server::admin::admin_router())
...
fn gpu_cores() -> u32 {
    crate::ecs::scheduling::HardwareConfig::detect().gpu_cores
}
...
async fn v1_chat_completions(
    ...
    match model_type {
        ModelType::Diffusion => {
            ...
            let chat_messages: Vec<crate::ecs::generation::diffusiongemma::ChatMessage> = messages
            .iter().filter_map(|msg| {
                let role = msg.get("role").and_then(|v| v.as_str())?;
                let content_val = msg.get("content")?;
                if let Some(text) = content_val.as_str() {
                    Some(crate::ecs::generation::diffusiongemma::ChatMessage {
                        role: role.to_string(),
                        content: vec![crate::ecs::generation::diffusiongemma::ContentPart::Text(text.to_string())],
                    })
                    ...
                    let content: Vec<_> = parts.iter().filter_map(|part| {
                        ...
                        match t {
                            "text" => part.get("text").and_then(|v| v.as_str())
                                .map(|s| crate::ecs::generation::diffusiongemma::ContentPart::Text(s.to_string())),
                            "image_url" => part.get("image_url").and_then(|v| v.get("url")).and_then(|v| v.as_str())
                                .map(|u| crate::ecs::generation::diffusiongemma::ContentPart::ImageUrl(u.to_string())),
                            "video_url" => part.get("video_url").and_then(|v| v.get("url")).and_then(|v| v.as_str())
                                .map(|u| crate::ecs::generation::diffusiongemma::ContentPart::VideoUrl(u.to_string())),
                            "audio_url" => part.get("audio_url").and_then(|v| v.get("url")).and_then(|v| v.as_str())
                                .map(|u| crate::ecs::generation::diffusiongemma::ContentPart::AudioUrl(u.to_string())),
                            _ => None,
        }
                    }).collect();
                    Some(crate::ecs::generation::diffusiongemma::ChatMessage { role: role.to_string(), content })
                } else { None }
            }).collect();
                ...
                body.get("tools").and_then(|v| v.as_array()).map(|tools| {
                    tools
                        ...
                        .filter_map(|t| {
                            ...
                            Some(crate::ecs::generation::diffusiongemma::ToolDefinition {
                ...
                if let Some(first) = m.first_mut() {
                    if first.role == "system" {
                        first.content.insert(
                            0,
                            crate::ecs::generation::diffusiongemma::ContentPart::Text(
...
async fn image_generations(
    ...
    let image_data = match model_name {
        "diffusiongemma" => {
            ...
            let dg = crate::ecs::generation::diffusiongemma::DiffusionGemmaGenerator::load(
                &sources
                    .get("diffusiongemma")
                    .and_then(|s| match s {
