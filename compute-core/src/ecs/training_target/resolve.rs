...
fn build_layout(entry: &Value) -> Result<PhysicalTileLayout, TrainingTargetResolveError> {
    ...
    if let Some(val) = entry.get("tile_family").and_then(|v| v.as_str()) {
        match val {
            "Tile640" | "tile640" => {
                layout.tile_family = crate::ecs::execution_profile::TileFamily::tile640();
                layout.tile_shape = crate::ecs::execution_profile::TileShape::tile640();
            }
            _ => {
    ...
    if let Some(val) = entry.get("group_axis").and_then(|v| v.as_str()) {
        match val {
            "PackedContiguous" | "packed_contiguous" => {
                layout.group_axis = crate::ecs::execution_profile::GroupAxis::PackedContiguous;
            }
            "OutputAxis" | "output_axis" => {
                layout.group_axis = crate::ecs::execution_profile::GroupAxis::OutputAxis;
            }
            "InputAxis" | "input_axis" => {
                layout.group_axis = crate::ecs::execution_profile::GroupAxis::InputAxis;
            }
            "TileLocal" | "tile_local" => {
                layout.group_axis = crate::ecs::execution_profile::GroupAxis::TileLocal;
            }
            _ => {
                return Err(TrainingTargetResolveError::InvalidLayoutField {
...
mod tests {
    ...
    fn training_layout_parses_from_policy() {
        ...
        assert_eq!(
            ...
            crate::ecs::execution_profile::GroupAxis::PackedContiguous
        );
        assert_eq!(
