...
pub type BackendTarget = crate::execution_plan::backend_capability::BackendLoweringTarget;
...
pub enum FusionSupportLevel { Full, Partial, Unsupported }
...
impl Default for FusionSelectionPolicy {
    fn default() -> Self {
        Self { prefer_lower_latency: true, prefer_memory_efficient: false, avoid_materialization: true }
    }
}
...
impl FusionScheduler {
    pub fn new(registry: BackendCapabilityRegistry) -> Self {
        Self { registry }
    }
    ...
    pub fn schedule(
        ...
        if graph.nodes.is_empty() {
            return Ok(FusionSchedule { groups: Vec::new(), receipts: Vec::new() });
        }
        ...
        for &node_idx in &topo {
            ...
            let body_indices = self.grow_group(graph, node_idx, &boundaries, &mut assigned, policy);
            if body_indices.is_empty() {
                continue;
            }

            let group = self.build_group(graph, &body_indices, group_counter);

            let (candidates, rejected) = self.evaluate_group(&group, role);

            let selected = if policy.allow_research_fusions {
                Self::score_select(&candidates, selection_policy)
            } else {
    ...
    fn grow_group(
        ...
        while let Some(candidate) = queue.pop() {
            if group.len() >= policy.max_group_size {
                if !group.contains(&candidate) { group.push(candidate); }
                break;
            }
            ...
            for buf in &node.outputs {
                for &consumer in &graph.consumers_of(buf) {
                    ...
                    if self.tentative_is_supported(graph, &tentative) {
    ...
    fn tentative_is_supported(&self, graph: &DataflowGraph, indices: &[usize]) -> bool {
        if indices.len() <= 1 { return true; }
        let tentative = self.build_group(graph, indices, usize::MAX);
        self.registry.all_targets().iter().any(|t| self.registry.supports(*t, &tentative).supported)
    }

    /// Build a FusedGroup from node indices.
    fn build_group(&self, graph: &DataflowGraph, indices: &[usize], id: usize) -> FusedGroup {
        ...
        for node in &body {
            for buf in &node.inputs {
                ...
                if prod.map_or(true, |p| !body_set.contains(&p)) {
                    if !inputs.contains(buf) { inputs.push(buf.clone()); }
                }
            }
            for buf in &node.outputs {
                let consumers = graph.consumers_of(buf);
                if !consumers.is_empty() && consumers.iter().all(|c| body_set.contains(c)) {
                    if !internal_values.contains(buf) { internal_values.push(buf.clone()); }
                }
                if consumers.iter().any(|c| !body_set.contains(c)) {
                    if !outputs.contains(buf) { outputs.push(buf.clone()); }
                }
            }
        }
    ...
    fn evaluate_group(
        ...
        for &target in &targets {
            let support = self.registry.evaluate(target, group, role);
            ...
            let cost = Self::compute_cost(group, target, &support);
            candidates.push(FusionCandidate { group: group.clone(), target, support: support_level, lowering_cost: cost });
        }
    ...
    fn compute_cost(group: &FusedGroup, _target: BackendLoweringTarget, support: &FusionSupport) -> LoweringCost {
        ...
        let bytes_read = support.estimated_memory_bytes.unwrap_or((op_count as u64) * 8192);
        let bytes_written = bytes_read;
        let scratch_bytes = support.estimated_scratch_bytes.unwrap_or(1024 * 1024);
        let materialization_cost = if support.requires_materialization { 10.0 } else { 0.0 };
        LoweringCost { estimated_us, bytes_read, bytes_written, scratch_bytes, thread_count: 256, materialization_cost }
    }

    /// Score-based selection using the policy.
    fn score_select(candidates: &[FusionCandidate], policy: &FusionSelectionPolicy) -> Option<FusionCandidate> {
        ...
        for c in candidates {
            ...
            if policy.prefer_memory_efficient {
                score -= ((c.lowering_cost.bytes_read + c.lowering_cost.bytes_written) as f64) / (1024.0 * 1024.0);
            }
            if policy.avoid_materialization && c.lowering_cost.materialization_cost > 0.0 {
                score -= c.lowering_cost.materialization_cost;
            }
            if score > best_score { best_score = score; best = Some(c); }
        }
        best.cloned()
    }
    ...
    fn prod_select(candidates: &[FusionCandidate]) -> Option<FusionCandidate> {
        candidates.iter().find(|c| c.support == FusionSupportLevel::Full)
            .or_else(|| candidates.iter().find(|c| c.support == FusionSupportLevel::Partial))
            .cloned()
    }
}
...
mod tests {
    ...
    fn schedule_is_deterministic() {
        ...
        let s1 = scheduler.schedule(&graph, &policy, &sel_policy, ProductionHotPath).unwrap();
        let s2 = scheduler.schedule(&graph, &policy, &sel_policy, ProductionHotPath).unwrap();
