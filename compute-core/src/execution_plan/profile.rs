...
#[cfg(all(target_os = "macos", feature = "metal-dispatch"))]
use super::region_encoder::RegionEncoder;

#[cfg(all(target_os = "macos", feature = "metal-dispatch"))]
use metal::{Device, ComputePipelineState};
        ...
        for region in &plan.regions {
            if let Ok(hazard) = HazardChecker::validate_region(region) {
                if hazard.safe {
                    // 2. Encode regions
                    for region in &plan.regions {
                        let _ = self.region_encoder.encode_region(region, &mut self.pso_cache);
                    }
                }
            }
        }
...
mod tests {
    ...
    async fn test_profile_run_result_default() {
        ...
        let result = run_profile_plan(&plan, &config, &mut sampler).await.unwrap();
