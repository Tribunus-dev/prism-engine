...
fn qkv_proj_region() -> FusionRegion {
    ...
    FusionRegion {
        id: "qkv_proj".into(),
        component_ops: vec!["q_proj".into(), "k_proj".into(), "v_proj".into()],
        input_layout,
        output_contract,
        implementations,
        fallback_decomposition: vec!["q_proj".into(), "k_proj".into(), "v_proj".into()],
...
fn gate_up_proj_region() -> FusionRegion {
    ...
    FusionRegion {
        ...
        component_ops: vec!["gate_proj".into(), "up_proj".into()],
        input_layout,
        output_contract,
        implementations,
        fallback_decomposition: vec!["gate_proj".into(), "up_proj".into()],
...
fn silu_mul_region() -> FusionRegion {
    ...
    FusionRegion {
        id: "silu_mul".into(),
        component_ops: vec!["silu".into(), "mul".into()],
        input_layout,
        output_contract,
        implementations,
        fallback_decomposition: vec!["silu".into(), "mul".into()],
        expected_eliminated_intermediates: 1,
    }
}
...
fn rms_norm_residual_region() -> FusionRegion {
    ...
    FusionRegion {
        ...
        component_ops: vec!["rms_norm".into(), "add".into()],
        input_layout,
        output_contract,
        implementations,
        fallback_decomposition: vec!["rms_norm".into(), "add".into()],
        expected_eliminated_intermediates: 1,
    }
}
...
mod tests {
    ...
    fn test_region_ids_are_unique() {
        let regions = generate_all_fusion_regions();
        let ids: std::collections::HashSet<&str> = regions.iter().map(|r| r.id.as_str()).collect();
        assert_eq!(ids.len(), 7);
    }
    ...
    fn test_serde_roundtrip() {
        ...
        for region in &regions {
            ...
            let parsed: FusionRegion = serde_json::from_str(&json).expect("deserialize");
            assert_eq!(parsed.id, region.id);
            assert_eq!(parsed.component_ops.len(), region.component_ops.len());
