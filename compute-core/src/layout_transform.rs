...
pub fn reorder_groups_neon(src: &[u8], k: usize, n: usize, group_size: usize) -> Vec<u8> {
    ...
    for ni in 0..n {
        for gi in 0..groups {
            ...
            dst[dst_off..dst_off + group_size].copy_from_slice(&src[src_off..src_off + group_size]);
        }
    }
...
pub fn verify_roundtrip(
    ...
    for i in 0..scales_orig.len() {
        if (s_rt[i] - scales_orig[i]).abs() > 1e-7 {
            return Err(format!(
                "scale mismatch at {}: orig={} rt={}",
                i, scales_orig[i], s_rt[i]
            ));
        }
    }
    ...
    for i in 0..biases_orig.len() {
        if (b_rt[i] - biases_orig[i]).abs() > 1e-7 {
            return Err(format!(
                "bias mismatch at {}: orig={} rt={}",
                i, biases_orig[i], b_rt[i]
            ));
        }
    }
...
mod tests {
    ...
    fn prepack_metadata_plausible() {
        ...
        assert!(
            meta.transform_us > 0,
            "transform should report non-zero time"
        );
    }
}
