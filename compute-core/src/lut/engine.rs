...
pub struct PrismEngine {
    ...
    #[cfg(all(target_os = "macos", any(feature = "mlx-backend", feature = "prism-backend")))] ane: Option<AneBackend>,
}
...
impl PrismEngine {
    pub fn load(path: &Path, graph: ModelGraph) -> Result<Self, String> {
        ...
        Ok(PrismEngine { graph, tensors,
            ...
            #[cfg(all(target_os = "macos", any(feature = "mlx-backend", feature = "prism-backend")))] ane: None,
        })
    }
    ...
    pub fn generate(&mut self, prompt: &[u32], mt: usize) -> Result<InferenceStats, String> {
        ...
        #[cfg(not(all(target_os = "macos", any(feature = "mlx-backend", feature = "prism-backend"))))]
        self.prefill_cpu(prompt, &mut kv, &mut pos)?;
