...
impl MatrixRole {
    ...
    pub fn is_attention(self) -> bool {
        matches!(self, Self::AttentionQ | Self::AttentionK | Self::AttentionV | Self::AttentionO)
    }
    ...
    pub fn is_boundary(self) -> bool {
        matches!(self, Self::Embedding | Self::LmHead | Self::MultimodalProjection)
    }

    /// Return true if this role is TTS/codec related.
    pub fn is_tts(self) -> bool {
        matches!(self, Self::TtsTalker | Self::TtsCodePredictor | Self::TtsCodec)
    }

    /// Default profile ID for this role.
    pub fn default_profile_id(self) -> crate::nf4tile640::profile::ProfileId {
        use crate::nf4tile640::profile::{
            PROFILE_ID_CANONICAL_NF4_V1, PROFILE_ID_GEMMA_ATTENTION_V1,
            PROFILE_ID_GEMMA_FFN_V1, PROFILE_ID_GEMMA_BOUNDARY_V1,
            PROFILE_ID_TTS_CODEC_V1,
        };
        if self.is_attention() { PROFILE_ID_GEMMA_ATTENTION_V1 }
        else if self.is_ffn() { PROFILE_ID_GEMMA_FFN_V1 }
        else if self.is_boundary() { PROFILE_ID_GEMMA_BOUNDARY_V1 }
        else if self.is_tts() { PROFILE_ID_TTS_CODEC_V1 }
        else { PROFILE_ID_CANONICAL_NF4_V1 }
    }
}
