        ...
        phases: vec![
            (
                ...
                Unsupported {
                    ...
                    reason: "no embedding primitive; needs host CPU or graph-runtime scheduler",
                },
            ),
            (
                PositionEncodingOrRope,
                Unsupported {
                    code: MissingPrimitive,
                    reason: "RoPE not available as single Accelerate primitive",
                },
            ),
            ...
            (
                ...
                Unsupported {
                    ...
                    reason: "KV cache is dynamic state; Accelerate is stateless kernel library",
                },
            ),
            (
                KvWrite,
                Unsupported {
                    code: StatefulBoundary,
                    reason: "KV cache mutation above Accelerate kernel library surface",
                },
            ),
            (
                KvAppend,
                Unsupported {
                    code: StatefulBoundary,
                    reason: "KV cache append is stateful; Accelerate is stateless",
                },
            ),
            (
                KvView,
                Unsupported {
                    code: StatefulBoundary,
                    reason: "Cache view assembly requires graph-level scheduler",
                },
            ),
            (
                AttentionScores,
                Unsupported {
                    code: NeedsGraphScheduling,
                    reason: "needs graph scheduling above BLAS to manage Q/K/V setup and batching",
                },
            ),
            (
                MaskApply,
                Unsupported {
                    code: NeedsGraphScheduling,
                    reason: "mask broadcast needs composite graph awareness",
                },
            ),
            (
                Softmax,
                Unsupported {
                    code: NeedsGraphScheduling,
                    reason:
                        "softmax requires elementwise+broadcast scheduling above Accelerate surface",
                },
            ),
            (
                AttentionWeightedSum,
                Unsupported {
                    code: NeedsGraphScheduling,
                    reason: "needs graph scheduling (QK^T result @ V) above BLAS",
                },
            ),
            ...
            (
                ...
                Unsupported {
                    code: MissingPrimitive,
                    reason: "RMS norm not available as single Accelerate primitive",
                },
            ),
            (MlpGateUp, Composed),
            (Activation, Composed),
            ...
            (
                SamplingOrLogitsPostprocess,
                Unsupported {
                    code: HostRuntimeResponsibility,
                    reason: "sampling is host-runtime operation",
                },
            ),
        ],
    }
...
pub fn decode_microphase_support_for(family: &str, backend: BackendKind) -> PhaseSupportStatus {
    let pending_mil = Pending {
        code: PendingCode::MilOpNotWired,
        reason: "decode microphase MIL builder not yet qualified",
    };
    let pending_bridge = Pending {
        code: PendingCode::BridgeNotQualified,
        reason: "decode microphase predict bridge not yet qualified",
    };
...
mod tests {
    ...
    fn comparison_grouping_requires_same_shape() {
        ...
        assert_eq!(
            groups.len(),
            2,
            "different shape profiles should produce separate groups"
        );
    }
    ...
    fn all_phases_count_is_21() {
        assert_eq!(
            ALL_PHASES.len(),
            21,
            "ALL_PHASES must have exactly 21 entries after KV phase addition"
        );
    }
    ...
    fn kv_contracts_for_small_v1_yields_eight_entries() {
        ...
        assert_eq!(
            contracts.len(),
            8,
            "expected 8 KV contracts for small_v1/mlx, got {}",
            contracts.len()
        );
    }
