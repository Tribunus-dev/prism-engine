...
pub fn generate_m1_throughput_profile() -> DeploymentProfile {
    DeploymentProfile {
        ...
        priority: 50,
        regions: vec!["us-west-1".into(), "us-east-1".into(), "eu-west-1".into()],
        chip_family: "m1".into(),
...
pub fn select_profile<'a>(
    ...
    if let Some(p) = profiles.iter().find(|p| p.workload_class == workload_class) {
        return Some(p);
    }
    ...
    profiles.iter().find(|p| p.workload_class == "fallback")
}
...
mod tests {
    ...
    fn test_candidate_class_serde() {
        let cases = vec![
            ...
            (
                CandidateClass::BenchmarkUnresolved,
                "\"benchmarkUnresolved\"",
            ),
        ];
