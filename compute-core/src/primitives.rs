...
pub fn quantized_embedding_lookup(
    ...
    let group_size = if n_groups > 0 {
        // Standard block-quant group size (64), derived below from packing ratio
        64
    } else {
        64
    };
    // Derive bits from the physical/logical packing ratio.
    // Each uint32 stores 32/bits logical values, so:
    //   packed_cols * (32/bits) = n_groups * group_size  (= logical_in)
    // Therefore: bits = 32 * packed_cols / (n_groups * group_size)
