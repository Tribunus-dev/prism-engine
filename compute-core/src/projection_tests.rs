...
mod tests {
        ...
        fn array(&self, name: &str) -> Array {
            ...
            match t.dtype() {
                ...
                Dtype::BF16 => {
                    ...
                    Array::from_slice(&u16_vec, &shape)
    ...
    fn check_parity(shard: &Shard, base: &str, in_dim: i32, out_dim: i32, label: &str) {
        ...
        let r = x
            .matmul(&mlx_rs::ops::transpose(&wf).expect("t"))
            .expect("matmul");
        ...
        let rel_err = if max_q > 0.0 {
            max_abs as f32 / max_q
        } else {
            0.0
        };
        println!(
            "{:30} |q|={:.4} |r|={:.4} max={:.2e} mean={:.2e} rel={:.2e} cos={:.8}",
        ...
        if max_abs > 1e-2 {
            println!(
                "  WARNING: {} max_err={:.2e} exceeds tolerance",
                label, max_abs
            );
        }
    }
    ...
    fn all_real_projection_parity() {
        ...
        check_parity(
            &s1,
            &format!("{}.layers.0.self_attn.q_proj", r),
            3840,
            4096,
            "sliding-q_proj-L0",
        );
        check_parity(
            &s1,
            &format!("{}.layers.12.self_attn.q_proj", r),
            3840,
            4096,
            "q_proj-L12",
        );
        check_parity(
            &s1,
            &format!("{}.layers.5.self_attn.q_proj", r),
            3840,
            8192,
            "full-q_proj-L5",
        );
        check_parity(
            &s1,
            &format!("{}.layers.0.mlp.gate_proj", r),
            3840,
            15360,
            "mlp-gate-L0",
        );
        check_parity(
            &s1,
            &format!("{}.layers.0.mlp.down_proj", r),
            15360,
            3840,
            "mlp-down-L0",
        );
        check_parity(
            &s1,
            &format!("{}.layers.0.self_attn.o_proj", r),
            4096,
            3840,
            "o_proj-L0",
        );
        check_parity(
            &s1,
            &format!("{}.layers.0.self_attn.k_proj", r),
            3840,
            2048,
            "k_proj-L0",
        );
