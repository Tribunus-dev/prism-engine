...
impl WorkerResponseRegistry {
    ...
    pub fn register(&self, request_id: &str, entity_id: u32, sink: Box<dyn Any + Send>) {
        let key = Self::key(request_id, entity_id);
        let mut guard = self
            .entries
            .lock()
            .expect("WorkerResponseRegistry lock poisoned");
        guard.insert(key, sink);
    }

    /// Remove and return the response sink for a `(request_id, entity_id)` pair.
    pub fn remove(&self, request_id: &str, entity_id: u32) -> Option<Box<dyn Any + Send>> {
        let key = Self::key(request_id, entity_id);
        let mut guard = self
            .entries
            .lock()
    ...
    pub fn deliver_token<T: 'static>(
        ...
        let sink = guard
            .get_mut(&key)
            .ok_or_else(|| format!("no sink registered for ({request_id}, {entity_id})"))?;
        let typed = sink
            .downcast_mut::<T>()
            .ok_or_else(|| format!("sink type mismatch for ({request_id}, {entity_id})"))?;
        deliver(typed)
    }
    ...
    pub fn deliver_terminal<T: 'static>(
        ...
        let sink = guard
            .remove(&key)
            .ok_or_else(|| format!("no sink registered for ({request_id}, {entity_id})"))?;
        let typed: Box<T> = sink
            .downcast::<T>()
            .map_err(|_| format!("sink type mismatch for ({request_id}, {entity_id})"))?;
        deliver(*typed)
    }
    ...
    pub fn register_pending(&self, request_id: &str, tx: std::sync::mpsc::Sender<String>) {
        let mut guard = self
            .pending
            .lock()
            .expect("WorkerResponseRegistry lock poisoned");
    ...
    pub fn deliver_response(&self, request_id: &str, response: String) -> Result<(), String> {
        let mut guard = self
            .pending
            .lock()
            .expect("WorkerResponseRegistry lock poisoned");
        let pending = guard
            .remove(request_id)
            .ok_or_else(|| format!("no pending request registered for '{request_id}'"))?;
