...
use tribunus_evidence_schema::{
    ArtifactPreparationBackend, ConditioningRecipe, ConditioningRecipeCompletionState,
    ConditioningRecipeEvent, ConditioningRecipeId, ExecutionConditioningPolicy, ModelReadiness,
    PrefetchLifecycleEvent, PrefetchLifecycleStage, ReadinessTransitionEvent, ResidencyGroup,
    ResourceId, SyntheticInputContract,
};
...
impl ConditioningExecutor {
    ...
    pub async fn execute_plan(
        ...
        for recipe in recipes {
            ...
            let completed_step_count = if completion == ConditioningRecipeCompletionState::Completed
            {
                step_count
            } else {
                0
    ...
    fn execute_step(
        ...
        let output = mlx_rs::ops::matmul(&input, &weight)
            .map_err(|e| format!("conditioning op failed for step {}: {e}", contract.step_id))?;
        self.scratch.store(output.clone());

        // ── 3. Force eval() to materialise the computation ────────────────
        output
            .eval()
            .map_err(|e| format!("eval failed for step {}: {e}", contract.step_id))?;
        ...
        //   one‑time warm‑up step, not a latency‑critical decode.
        {
            ...
            let flat = cast
                .try_as_slice::<f32>()
                .map_err(|e| format!("failed to read output for step {}: {e}", contract.step_id))?;
...
impl PrefetchCoordinator {
    ...
    pub fn submit_next_layer(&self, group: ResidencyGroup) -> Result<(), String> {
        ...
        self.tx.try_send(req).map_err(|e| match e {
            mpsc::error::TrySendError::Full(_) => "prefetch channel full (capacity 2)".to_string(),
...
impl ReadinessManager {
    ...
    fn apply_transition(
        ...
        let now = Instant::now();
        let transition_ns = now.duration_since(self.last_transition_at).as_nanos() as u64;
...
impl TreatmentSummary {
    ...
    pub fn record_conditioning_event(&mut self, event: &ConditioningRecipeEvent) {
        self.total_conditioning_ns = self
            .total_conditioning_ns
            .saturating_add(event.total_conditioning_ns);
        if event.completion == ConditioningRecipeCompletionState::Completed {
            self.completed_steps = self
                .completed_steps
                .saturating_add(event.completed_step_count);
        } else {
    ...
    pub fn record_prefetch_event(&mut self, event: &PrefetchLifecycleEvent) {
        ...
        self.total_prefetch_bytes = self
...
mod tests {
    ...
    use tribunus_evidence_schema::{
        ...
        ConditioningRecipeCompletionState, ConditioningRecipeId, ExpectedSubstrate,
        PipelinePlanVersion, ResidencyGroupId, ResidencyPlanVersion, ResidencyPriority,
    };
