...
pub fn sample_process_rss(pid: u32) -> u64 {
    #[cfg(target_os = "macos")]
    {
        ...
        let ret = unsafe {
            libc::proc_pid_rusage(pid as libc::c_int, libc::RUSAGE_INFO_V2, &mut info_ptr)
        };
        if ret == 0 {
...
pub fn sample_process_rss_self() -> u64 {
    ...
    {
        use libc::{
            mach_task_basic_info, task_info, MACH_TASK_BASIC_INFO, MACH_TASK_BASIC_INFO_COUNT,
        };
        unsafe extern "C" {
...
mod tests {
    ...
    fn test_detect_profile() {
        ...
        {
            assert!(
                p.total_physical_bytes > 0,
                "total_physical_bytes should be > 0 on macOS"
            );
            assert!(p.usable_bytes > 0, "usable_bytes should be > 0 on macOS");
            assert!(
                p.usable_bytes <= p.total_physical_bytes,
                "usable must not exceed total"
            );
            assert!(
                !p.model.is_empty(),
                "model string should not be empty on macOS"
            );
            assert!(
                !p.gpu_family.is_empty(),
                "gpu_family should not be empty on macOS"
            );
        }
    ...
    fn test_usable_reserve() {
        ...
        assert_eq!(
            usable_after_system_reserve(16 * 1024 * 1024 * 1024),
            14 * 1024 * 1024 * 1024
        );
        // 8 GiB → 7 GiB usable (1 GiB reserve).
        assert_eq!(
            usable_after_system_reserve(8 * 1024 * 1024 * 1024),
            7 * 1024 * 1024 * 1024
        );
        // 32 GiB → 28 GiB usable (4 GiB reserve).
        assert_eq!(
            usable_after_system_reserve(32 * 1024 * 1024 * 1024),
            28 * 1024 * 1024 * 1024
        );
        // 64 GiB → 56 GiB usable (8 GiB reserve).
        assert_eq!(
            usable_after_system_reserve(64 * 1024 * 1024 * 1024),
            56 * 1024 * 1024 * 1024
        );
