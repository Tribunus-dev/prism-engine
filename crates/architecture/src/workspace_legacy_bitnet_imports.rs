//! `workspace_contains_no_legacy_bitnet_imports`
//!
//! Workspace-level architecture enforcement test for the
//! bitnet migration. It scans the workspace for any `use`
//! statement that references the legacy engine bitnet
//! surface (`compute_core::ecs::bitnet::*` or
//! `crate::ecs::bitnet::*` inside the engine).
//!
//! The test fails if any file in the workspace imports the legacy
//! surface. The legacy engine file is the engine's
//! `compute-core/src/ecs/bitnet/` directory; the
//! constitutional replacement is `prism_ecs_quantization::bitnet`.
//!
//! # Migration status
//!
//! This test was added when the bitnet engine-deletion
//! migration was being completed (2026-07-27). The engine's
//! `compute-core/src/ecs/bitnet/` directory will be deleted
...
/// Returns the set of files OUTSIDE the legacy engine bitnet
/// directory that import the legacy surface. Files inside the
/// engine's own `compute-core/src/ecs/bitnet/` directory
...
pub fn legacy_bitnet_importers_outside_inventory() -> Vec<String> {
    let mut importers = Vec::new();
    scan_workspace_excluding_inventory("compute-core", &mut importers);
    importers
}

const LEGACY_INVENTORY_DIR: &str = "compute-core/src/ecs/bitnet";
    ...
    for entry in entries.flatten() {
        ...
        if p.is_dir() {
            ...
            if let Ok(content) = fs::read_to_string(&p) {
                if content.contains("use crate::ecs::bitnet::")
                    || content.contains("compute_core::ecs::bitnet::")
                    || content.contains("tribunus_compute_core::bitnet")
                {
...
mod tests {
    ...
    #[test]
    fn workspace_contains_no_legacy_bitnet_imports() {
        // Architectural invariant: no file OUTSIDE the migration
        // inventory imports the legacy engine bitnet
        // surface. The engine's
        // compute-core/src/ecs/bitnet/ directory is the
