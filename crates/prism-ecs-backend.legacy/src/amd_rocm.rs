//! AMD ROCm backend — [`TensorBackend`] implementation for AMD GPUs.
//!
//! This backend targets AMD discrete GPUs on Linux via the ROCm platform
//! (HIP runtime).  In the initial stub implementation (compiled on any
//! platform with `feature = "amd-rocm"`), all compute operations return
//! errors.  Tensor creation, shape inspection, and reference-counted
//! lifecycle use a generational slot-map identical to
//! [`IntelLevelZeroBackend`](crate::intel_level_zero::IntelLevelZeroBackend).
//!
//! # Feature gate
//!
...
use crate::DType as BackendDType;
use crate::{
...
/// AMD GPU compute backend using ROCm / HIP.
///
/// Uses generational slot-maps identical to [`IntelLevelZeroBackend`] and
/// [`MlxBackend`].  In stub mode, tensor storage and lifecycle are backed
/// by `Vec<f32>` arrays; compute operations return errors.
///
/// When HIP FFI is wired in (Phase 2), tensors will live in
/// hipMalloc'd device memory accessible from both CPU and the AMD GPU
/// via hipMemcpy / hipMemPrefetchAsync, enabling zero-copy dispatch.
pub struct AmdRocmBackend {
...
impl AmdRocmBackend {
    ...
    fn get_storage(&self, handle: TensorHandle) -> Result<&[f32], String> {
        ...
        match self.storages.get(slot) {
            ...
            _ => Err(format!(
                "AmdRocm: invalid tensor handle (slot={}, gen={})",
                slot, generation,
            )),
        }
    }
...
impl Default for AmdRocmBackend {
    fn default() -> Self {
        Self::new()
    }
}

impl TensorBackend for AmdRocmBackend {
    // ── Creation ───────────────────────────────────────────────────────
    ...
    fn quantized_matmul(
        ...
        Err("AmdRocm: quantized_matmul not implemented in stub mode".into())
    }
    ...
    fn matmul(
        ...
        Err("AmdRocm: matmul not implemented in stub mode".into())
    }
    ...
    fn index_select(
        ...
        Err("AmdRocm: index_select not implemented in stub mode".into())
    }
    ...
    fn concatenate(
        ...
        Err("AmdRocm: concatenate not implemented in stub mode".into())
    }
    ...
    fn slice(
        ...
        Err("AmdRocm: slice not implemented in stub mode".into())
    }

    fn cast(&mut self, _x: TensorHandle, _dtype: BackendDType) -> Result<TensorHandle, String> {
        Err("AmdRocm: cast not implemented in stub mode".into())
    }
    ...
    fn evaluate(
        ...
        Ok(EvaluationReceipt {
            ...
            observed_substrate: Some("amd-rocm-stub".to_string()),
...
mod tests {
    ...
    fn make_f32(backend: &mut AmdRocmBackend, data: &[f32], shape: &[i32]) -> TensorHandle {
        backend.create_f32(data, shape).unwrap()
    }

    /// Helper to read back an f32 tensor handle.
    fn read_f32(backend: &mut AmdRocmBackend, h: TensorHandle) -> Vec<f32> {
        backend.read_f32(h).unwrap().data
    }

    #[test]
    fn test_create_and_read_f32() {
        let mut backend = AmdRocmBackend::new();
    ...
    fn test_backend_capabilities() {
        let backend = AmdRocmBackend::new();
        ...
        assert_eq!(caps.backend_name, "amd-rocm");
    }
    ...
    fn test_evaluate() {
        ...
        assert_eq!(receipt.observed_substrate.as_deref(), Some("amd-rocm-stub"));
    }
