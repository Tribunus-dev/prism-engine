...
use crate::flex_dispatch::FlexDispatch;
use crate::routing::*;
use crate::CompiledRegionBackend;
use crate::DType;
use crate::TensorBackend;
use crate::TensorHandle;
use tribunus_compute_core::arena::Arena;
#[cfg(feature = "mlx-backend")]
use tribunus_compute_core::memory::allocator::IosurfaceAllocator;
...
pub trait BackendInstance: TensorBackend + Send + Sync {
    ...
    fn evaluate_into_arena(
        ...
        _arena: &Arena,
    ) -> Result<crate::EvaluationReceipt, String> {
                ...
                for op_desc in &ops {
                    ...
                    if is_compiled_region {
                        if let Some(compiled_backend) = _backend.as_compiled_region_backend() {
                            ...
                            // Build a GraphRegion from the operation descriptor
                            let mut region = crate::routing::GraphRegion {
...
mod tests {
    use super::*;
    use crate::{DType, MatmulOp, QuantizedMatmulOp, RmsNormOp, RoPEOp};
    ...
    impl TensorBackend for TestBackend {
        ...
        fn quantized_matmul(
            ...
            _x: TensorHandle,
            _w: crate::QuantizedWeightHandle,
        ...
        fn read_f32(&mut self, _handle: TensorHandle) -> Result<crate::ReadbackReceipt, String> {
            Err("stub".into())
        }
        ...
        fn backend_capabilities(&self) -> crate::BackendCapabilities {
    ...
    impl BackendInstance for TestBackend {
        ...
        fn execute(
            ...
            Ok(BackendExecutionReceipt {
                ...
                backend_version: crate::routing::BackendVersion {
                    backend_name: "test".into(),
                    version: "0.0.0".into(),
                    git_commit: None,
                },
