use prism_ecs_columnar::aggregate::{
    avg, count, filtered_rows, histogram, max, min, quantile, sum, FilterExpr,
};
use prism_ecs_columnar::columnar::{append_row, create_table, ColumnarTable};
use prism_ecs_columnar::projection::{
    materialize, refresh_projections, AggExpr, Projection, ProjectionEngine, ProjectionQuery,
};
use prism_ecs_columnar::types::{DuckType, DuckValue};
...
/// Test: quantile on [1,2,3,4,5] at p=0.0 returns 1 (min)
#[test]
...
/// Test: quantile on [1,2,3,4,5] at p=1.0 returns 5 (max)
#[test]
...
fn test_histogram_even() {
    ...
    // With equal-width buckets on [1..100], each bucket gets ~10 values.
    // Due to float bin boundaries some may be 9-11, but 10 each on exact ints
    for (i, &count) in hist.iter().enumerate() {
        assert!(
            count == 9 || count == 10 || count == 11,
            "bucket {i}: expected ~10, got {count}"
        );
    }
}
...
fn test_projection_roundtrip() {
    ...
    // Aggregated: 1 row with count=3 and avg=85
