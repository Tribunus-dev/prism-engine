...
/// Phase DAG logical tensor identifier (the constitutional
/// `prism_ecs_core` crate does not yet expose a public
/// `LogicalTensorId` newtype; the engine's
/// `ecs::canonical::identity::LogicalTensorId` is a `String`
/// newtype and the constitutional surface uses the same shape).
