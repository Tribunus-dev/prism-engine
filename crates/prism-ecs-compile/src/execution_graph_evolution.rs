...
impl TargetExecutionGraph {
    pub fn canonical_digest(&self) -> String {
        ...
        canonical.edges.sort_by(|a, b| {
            (&a.from_unit, &a.to_unit, &a.buffer).cmp(&(&b.from_unit, &b.to_unit, &b.buffer))
        });
    ...
    pub fn verify(&self) -> Result<(), ExecutionGraphError> {
        if self.graph_id.is_empty()
            || self.target_profile_digest.is_empty()
            || self.semantic_region_plan_digest.is_empty()
        {
        ...
        if self.edges.iter().any(|edge| {
            !unit_ids.contains(edge.from_unit.as_str()) || !unit_ids.contains(edge.to_unit.as_str())
        }) {
...
pub fn mutate_execution_graph(
    ...
    match mutation {
        ExecutionGraphMutation::FuseAdjacent if candidate.units.len() >= 2 => {
            ...
            candidate.units[0]
                .semantic_region_ids
                .extend(right.semantic_region_ids);
            candidate
                .edges
                .retain(|edge| edge.from_unit != right.unit_id && edge.to_unit != right.unit_id);
        }
        ExecutionGraphMutation::MoveToMetal => {
            candidate.units[0].kind = ExecutableUnitKind::MetalKernel
        }
        ExecutionGraphMutation::MoveToAccelerate => {
            candidate.units[0].kind = ExecutableUnitKind::AccelerateProgram
        }
        ExecutionGraphMutation::MoveToMlModelc => {
            candidate.units[0].kind = ExecutableUnitKind::MlModelcProgram
        }
        ExecutionGraphMutation::MoveToCpu => {
            candidate.units[0].kind = ExecutableUnitKind::CpuKernel
        }
        ExecutionGraphMutation::InsertConversion => {
            candidate.units.push(ExecutableUnitCandidate {
                unit_id: format!("conversion-{}", candidate.units.len()),
                kind: ExecutableUnitKind::ConversionKernel,
                operation_ids: vec!["convert".into()],
                semantic_region_ids: vec![],
                input_layouts: vec![],
                output_layouts: vec![],
        ...
        other => {
            candidate.units[0]
                .compile_options
                .insert("last_mutation".into(), format!("{other:?}"));
        }
    }
    candidate.seal()
}
...
pub fn admit_execution_graph(
    ...
    if !measurement.measured
        || measurement.execution_fingerprint.is_empty()
        || measurement.graph_digest != graph.graph_digest
    {
        return Err(ExecutionGraphError::Unmeasured);
    }
    let mut reasons = Vec::new();
    if measurement.behavioral_quality < policy.min_behavioral_quality {
        reasons.push("behavioral quality below gate".into());
    }
    if measurement.agentic_success < policy.min_agentic_success {
        reasons.push("agentic success below gate".into());
    }
    if let Some(limit) = policy.max_prefill_latency_ms {
        if measurement.prefill_latency_ms > limit {
            reasons.push("prefill latency exceeds gate".into());
        }
    }
    if let Some(limit) = policy.max_decode_latency_ms {
        if measurement.decode_latency_ms > limit {
            reasons.push("decode latency exceeds gate".into());
        }
    }
    if let Some(limit) = policy.min_tokens_per_second {
        if measurement.tokens_per_second < limit {
            reasons.push("throughput below gate".into());
        }
    }
    if let Some(limit) = policy.max_peak_resident_bytes {
        if measurement.peak_resident_bytes > limit {
            reasons.push("resident bytes exceed gate".into());
        }
    }
    if let Some(limit) = policy.max_copy_bytes {
        if measurement.copy_bytes > limit {
            reasons.push("copy bytes exceed gate".into());
        }
    }
    if let Some(limit) = policy.max_conversion_bytes {
        if measurement.conversion_bytes > limit {
            reasons.push("conversion bytes exceed gate".into());
        }
    }
    if let Some(limit) = policy.max_synchronization_ms {
        if measurement.synchronization_ms > limit {
            reasons.push("synchronization cost exceeds gate".into());
        }
    }
    if let Some(limit) = policy.max_kernel_count {
        if measurement.kernel_count > limit {
            reasons.push("kernel count exceeds gate".into());
        }
    }
    if measurement.fallback_frequency > policy.max_fallback_frequency {
        reasons.push("fallback frequency exceeds gate".into());
    }
    if measurement.receipt_completeness < policy.min_receipt_completeness {
        reasons.push("receipt completeness below gate".into());
    }
    let mut receipt = ExecutionGraphAdmissionReceipt {
