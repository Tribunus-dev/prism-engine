...
impl SemanticRegionDiscoverer for GraphExplicitDiscoverer {
    fn discover(
        ...
        for hint in graph_hints {
            match hint {
                GraphRegionHint::AxisSplit { operation, source_value, axis, spans } => {
                    for (start, end, role) in spans {
                        regions.push(descriptor(
                            ...
                            *end,
                            ...
                            vec![format!("graph-op:{operation}"), format!("source:{source_value}")],
                        ));
                    }
                }
            }
...
pub fn discover_semantic_partition(
    ...
    let regions = if regions.is_empty() {
        vec![descriptor(
            ...
            0,
            tensor.shape[0],
            RegionRole::Generic { label: "whole_tensor".into() },
...
fn discover_qkv(
    ...
    let heads = model.num_attention_heads.ok_or(SemanticDiscoveryError::InvalidQkvDimensions)? as u64;
    let kv_heads = model.num_key_value_heads.unwrap_or(heads as u32) as u64;
    let head_dim = model.head_dim.ok_or(SemanticDiscoveryError::InvalidQkvDimensions)? as u64;
    ...
    Ok(vec![
        descriptor(tensor, 0, 0, q, RegionRole::QueryProjection, origin("fused-qkv-gqa"), vec!["config:num_attention_heads".into(), "config:head_dim".into()]),
        descriptor(tensor, 0, q, q + k, RegionRole::KeyProjection, origin("fused-qkv-gqa"), vec!["config:num_key_value_heads".into(), "config:head_dim".into()]),
        descriptor(tensor, 0, q + k, q + k + v, RegionRole::ValueProjection, origin("fused-qkv-gqa"), vec!["config:num_key_value_heads".into(), "config:head_dim".into()]),
    ])
}
...
fn discover_gate_up(
    ...
    let intermediate = model.intermediate_size.ok_or(SemanticDiscoveryError::InvalidGateUpDimensions)?;
    ...
    Ok(vec![
        descriptor(tensor, 0, 0, intermediate, RegionRole::GateProjection, origin("fused-gate-up"), vec!["config:intermediate_size".into()]),
        descriptor(tensor, 0, intermediate, intermediate * 2, RegionRole::UpProjection, origin("fused-gate-up"), vec!["config:intermediate_size".into()]),
    ])
}
...
fn descriptor(
    ...
    let id = format!("sr:{}:axis:{axis}:{start}:{end}:{:x}", short_digest(&tensor.id.0), hasher.finalize());
    SemanticRegionDescriptor {
        ...
        constraints: RegionConstraints {
            allowed_formats: vec!["fp16".into(), "bf16".into(), "int8".into(), "nf4".into(), "ternary158".into()],
...
mod tests {
    ...
    fn tensor(name: &str, rows: u64) -> LogicalTensorDescriptor {
        LogicalTensorDescriptor { id: LogicalTensorId(name.into()), name: name.into(), shape: vec![rows, 2048] }
    }

    #[test]
    fn discovers_gqa_qkv_without_equal_thirds() {
        let model = SemanticModelConfig { model_family: "qwen".into(), num_attention_heads: Some(32), num_key_value_heads: Some(8), head_dim: Some(128), intermediate_size: None };
        let p = discover_semantic_partition(&tensor("layer.qkv_proj.weight", 6144), &model, &[]).unwrap();
        assert_eq!(p.regions.len(), 3);
        assert!(matches!(p.regions[1].selector, RegionSelector::AxisSpan { start: 4096, end: 5120, .. }));
    }

    #[test]
    fn discovers_gate_up_split() {
        let model = SemanticModelConfig { model_family: "generic".into(), intermediate_size: Some(4096), ..Default::default() };
        let p = discover_semantic_partition(&tensor("mlp.gate_up_proj.weight", 8192), &model, &[]).unwrap();
        assert!(matches!(p.regions[1].role, RegionRole::UpProjection));
    }

    #[test]
    fn graph_hint_precedes_architecture_rule() {
        let hints = vec![GraphRegionHint::AxisSplit { operation: "split-17".into(), source_value: "v42".into(), axis: 0, spans: vec![(0, 2, RegionRole::QueryProjection), (2, 4, RegionRole::KeyProjection)] }];
        let p = discover_semantic_partition(&tensor("custom.weight", 4), &SemanticModelConfig::default(), &hints).unwrap();
        assert!(matches!(p.regions[0].origin, RegionOrigin::GraphDerived { .. }));
    }

    #[test]
    fn unsupported_tensor_falls_back_to_whole_tensor() {
        let p = discover_semantic_partition(&tensor("norm.weight", 2048), &SemanticModelConfig::default(), &[]).unwrap();
