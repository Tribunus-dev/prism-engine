...
//! `EnginePool` / `EngineEntry` / `EngineLifecycle` types that
//! decide whether a new model can be loaded given a memory budget
//! and evict idle engines when pressure rises. The pool is
//! engine-independent — no `unsafe`, no platform FFI — and is
//! ready for replay and projection rebuild from the same durable
//! history.
...
impl EnginePool {
    ...
    pub fn mark_idle(&mut self, id: &str) {
        if let Some(entry) = self.engines.iter_mut().find(|e| e.id == id) {
    ...
    pub fn evict_idle(&mut self) -> Option<String> {
        self.engines
            .iter()
            .filter(|e| e.state == EngineLifecycle::Idle)
