    ...
    match table.columns[col].dtype() {
        ...
        DuckType::Integer => {
            ...
            let mut values: Vec<f64> = c
                ...
                .map(|(v, _)| *v as f64)
                .collect();
            quantile_sorted_f64(&mut values, p)
        }
        DuckType::BigInt => {
            let c = any.downcast_ref::<Column<i64>>().ok_or("type mismatch")?;
            let mut values: Vec<f64> = c
...
fn quantile_f64(data: &[f64], nulls: &[bool], p: f64) -> Result<DuckValue, String> {
...
fn quantile_sorted_f64(values: &mut Vec<f64>, p: f64) -> Result<DuckValue, String> {
    if values.is_empty() {
        return Err("empty column".to_string());
    }
