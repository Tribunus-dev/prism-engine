...
use crate::evolution::foundation::{CandidateGenome, GenomeAxis, GenomeAxisSet, GENOME_AXIS_COUNT};
...
impl DecompositionSystem {
    ...
    pub fn run(&self, genome: &CandidateGenome) -> DecompositionResult {
        ...
        let avg_fitness = sub_problems
            .iter()
            .map(|sp| sp.active_axes.count() as f64 / GENOME_AXIS_COUNT as f64)
            .sum::<f64>()
...
mod tests {
    ...
    fn decompose_full_genome() {
        let problems = DecompositionSystem::default().decompose(&CandidateGenome::new());
        assert_eq!(problems.len(), 4);
        assert_eq!(problems[0].name, "tile-geometry-decomp");
        assert!(problems[0].active_axes.contains(GenomeAxis::MetalGeometry));
    }
    #[test]
    fn decomposition_disabled_returns_single_problem() {
        let system = DecompositionSystem::new(DecompositionConfig {
            enabled: false,
            max_sub_problems: 4,
        });
        let problems = system.decompose(&CandidateGenome::new());
        assert_eq!(problems.len(), 1);
        assert_eq!(problems[0].active_axes.count() as usize, GENOME_AXIS_COUNT);
    }
    #[test]
    fn decomposition_result_has_fitness() {
        let result = DecompositionSystem::default().run(&CandidateGenome::new());
