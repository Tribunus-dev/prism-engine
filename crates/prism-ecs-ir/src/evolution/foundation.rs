...
impl GenomeAxisSet {
    ...
    pub const fn all() -> Self {
        Self((1u16 << GENOME_AXIS_COUNT) - 1)
    }
    pub const fn from_axis(axis: GenomeAxis) -> Self {
        Self(1u16 << axis as u8)
    }
    pub const fn union(self, other: Self) -> Self {
        Self(self.0 | other.0)
    }
    pub const fn contains(self, axis: GenomeAxis) -> bool {
        (self.0 & (1u16 << axis as u8)) != 0
    }
...
mod tests {
    ...
    fn fitness_score_clamping() {
        assert_eq!(FitnessScore::new(1.5).value(), 1.0);
        assert_eq!(FitnessScore::new(-0.5).value(), 0.0);
    }
    #[test]
    ...
    #[test]
    fn all_axes_are_representable() {
        let all = GenomeAxisSet::all();
        assert_eq!(all.count() as usize, GENOME_AXIS_COUNT);
        assert!(all.contains(GenomeAxis::AneUnit));
    }
}
