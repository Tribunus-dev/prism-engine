...
//!
//! The importer stores all weight tensors in `[in_features, out_features]`
//! layout (rows = in, cols = out).  This is the transpose of the layout
//! that `ternary_gemv_reference` expects ([out, in]).  We therefore
//! provide our own `bitnet_linear` function that correctly computes
//! `output = input @ weight` for `[in, out]` storage.

use super::ternary_codec::{unpack_ternary_codes, TernaryCodecError, TernaryPackedTensor};
...
///
/// Processes `seq_len` tokens through the full attention + MLP block
...
mod tests {
    use super::*;
    use crate::bitnet::ternary_codec::pack_ternary_codes;
    use half::f16;

    fn norm_tensor(dim: usize) -> TernaryPackedTensor {
        let codes = pack_ternary_codes(&vec![1_i8; dim]).unwrap();
        TernaryPackedTensor {
            rows: 1,
            cols: dim,
            group_size: dim,
    ...
    fn proj_tensor(rows: usize, cols: usize, group_size: usize) -> TernaryPackedTensor {
        ...
        let codes = pack_ternary_codes(&weights).unwrap();
    ...
    fn make_test_tensors(
        ...
        vec![
            norm_tensor(hidden_dim),
            proj_tensor(hidden_dim, hidden_dim, group_size),
            proj_tensor(hidden_dim, kv_inner, group_size),
            proj_tensor(hidden_dim, kv_inner, group_size),
            proj_tensor(hidden_dim, hidden_dim, group_size),
            norm_tensor(hidden_dim),
            proj_tensor(hidden_dim, intermediate_dim, group_size),
            proj_tensor(hidden_dim, intermediate_dim, group_size),
            proj_tensor(intermediate_dim, hidden_dim, group_size),
            position_ids_tensor(seq_len),
            norm_tensor(hidden_dim),
        ]
    }
    ...
    #[test]
    fn decoder_layer_reference_runs() {
        let hidden_dim = 64;
        let num_heads = 4;
        let num_kv_heads = 2;
    ...
    #[test]
    fn decoder_layer_with_kv_cache() {
        let hidden_dim = 32;
        let num_heads = 4;
        let num_kv_heads = 2;
        let head_dim = 8;
