...
use prism_ecs_core::backend::accelerate_ffi::{cblas_sgemm, CBLAS_NO_TRANS, CBLAS_ROW_MAJOR};
...
pub fn validate_weight_space(
    ...
    WeightValidationReport {
        rmse,
        nrmse,
        max_abs_error: max_abs_err as f64,
        zero_collapse_ratio,
    }
}
...
pub fn validate_operator_space_with_vectors(
    ...
    ))]
    {
        ...
        use prism_ecs_core::compute_image::compile::kernel_dispatch::GpuBatchMatmulDispatcher;
        use prism_ecs_core::compute_image::compile::kernel_registry::KernelRegistry;
        use std::sync::Arc;
        ...
        let ref_results =
            GPU_INIT.run(source, &inputs_flat, in_features, out_features, num_vectors);
        let mut quant_results = GPU_INIT.run(
            &quant_weights,
            &inputs_flat,
            in_features,
            out_features,
        ...
        return ValidationOutcome::Completed(compute_operator_metrics(
            &ref_results,
            &quant_results,
            num_vectors,
            out_features,
    ...
    for chunk in vectors.chunks(VALIDATION_BATCH_SIZE) {
        for input in chunk {
            ...
            for j in 0..out_features {
                ...
                if diff_abs as f64 > max_abs_err {
                    max_abs_err = diff_abs as f64;
                }
                sq += diff * diff;
