...
/// GET /v1/sessions/{id} - get session state.
#[cfg(all(feature = "server", not(feature = "prism-backend")))]
async fn get_session(State(_server): State<AppState>, Path(id): Path<String>) -> Json<Value> {
    Json(json!({
        "status": "ok",
        "session_id": id,
        "state": "unknown"
...
async fn generate_audio(State(server): State<AppState>, Json(body): Json<Value>) -> Json<Value> {
    ...
    let params = crate::runtime::modality::AudioParams {
...
async fn generate_video(State(server): State<AppState>, Json(body): Json<Value>) -> Json<Value> {
    let model_path = body
        ...
        .unwrap_or("default");
    let prompt = body.get("prompt").and_then(|v| v.as_str()).unwrap_or("");
    let params = crate::runtime::modality::VideoParams {
        num_frames: body
            .get("num_frames")
            .and_then(|v| v.as_u64())
            .unwrap_or(16) as u32,
