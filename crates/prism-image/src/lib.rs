// ── Prism Image Generation — Crate Root ──────────────────────────────────
//
...
pub mod diffusion_gemma {
    ...
    impl DgCompiler {
        pub fn compile(_gguf_path: &Path, _variant: DgVariant) -> anyhow::Result<Metadata> {
            Ok(Metadata {})
        }
    }
}
...
fn generate_via_admission_pipeline(
    ...
    let cancellation_token = reliability::ImageGenerationCancellationToken::new(RequestId::new());
...
fn resolve_cimage(path: &str) -> Result<manifest::InstalledCImage, ImageGenerationError> {
    use crate::manifest::{ImageGenerationCapabilityManifest, InstalledCImage};
