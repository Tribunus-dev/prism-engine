...
pub enum TilingValidationError {
    ...
    #[error("{backend:?} tile dimensions {width}x{height} exceed the maximum {max_width}x{max_height}")]
    DimensionsTooLarge { backend: TilingBackend, width: usize, height: usize, max_width: usize, max_height: usize },
    #[error("{backend:?} tile area {area} exceeds the maximum of {max_area}")]
    AreaTooLarge { backend: TilingBackend, area: usize, max_area: usize },
}
...
pub fn validate_tiling_geometry(
    ...
    if geometry.width == 0 || geometry.height == 0 {
        return Err(TilingValidationError::ZeroDimension { width: geometry.width, height: geometry.height });
    }
    ...
    if geometry.width > max_width || geometry.height > max_height {
        return Err(TilingValidationError::DimensionsTooLarge { backend, width: geometry.width, height: geometry.height, max_width, max_height });
    }
    let area = geometry.width.checked_mul(geometry.height).unwrap_or(usize::MAX);
    if area > max_area {
        return Err(TilingValidationError::AreaTooLarge { backend, area, max_area });
    }
    Ok(area)
}

pub fn validate_joint_tiling_geometry(geometry: TileGeometry) -> Result<usize, TilingValidationError> {
