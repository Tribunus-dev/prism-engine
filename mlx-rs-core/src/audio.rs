...
pub fn resample(samples: &[f32], src_rate: u32, target_rate: u32) -> Vec<f32> {
    ...
    let mut resampler = match SincFixedIn::<f32>::new(
        ratio, 2.0, // Max relative delay
        params, chunk_size, // Input chunk size
        1,          // Mono channel
    ) {
        Ok(r) => r,
        Err(e) => {
    ...
    while pos + chunk_size <= samples.len() {
        ...
        match resampler.process(&chunk, None) {
            ...
            Err(e) => {
                eprintln!(
                    "Warning: Sinc resampling failed at chunk {}: {}. Falling back to linear.",
                    pos, e
                );
                return resample_linear(samples, src_rate, target_rate);
            }
...
fn stft_magnitude(samples: &[f32], n_fft: i32, hop_length: i32, win_length: i32) -> Vec<f32> {
...
pub fn compute_mel_spectrogram(samples: &[f32], config: &AudioConfig) -> Result<Array, Exception> {
    let n_freqs = (config.n_fft / 2 + 1) as usize;

    // Compute STFT magnitude [n_freqs, n_frames]
    let stft_mag = stft_magnitude(samples, config.n_fft, config.hop_length, config.win_length);
...
pub fn load_audio_for_hubert(path: impl AsRef<Path>) -> Result<Array, Box<dyn std::error::Error>> {
    // Load WAV
