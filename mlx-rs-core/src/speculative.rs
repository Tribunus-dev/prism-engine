    ...
    C: KeyValueCache + Default,
{
    ...
    fn generate_draft_tokens(&mut self, start_token: &Array) -> Result<Vec<Array>, Exception> {
        ...
        for _ in 0..self.num_draft_tokens {
            ...
            let logits = self
                .draft_model
                .forward_speculative(&input, self.draft_cache)?;
    ...
    fn verify_draft_tokens(
        ...
        let logits = self
            .target_model
            .forward_speculative(input_tokens, self.target_cache)?;
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        match &self.state {
            SpeculativeState::Prefill { prompt } => {
                ...
                let target_logits = match self
                    .target_model
                    .forward_speculative(prompt, self.target_cache)
                {
                    Ok(l) => l,
                ...
                if let Err(e) = self
                    .draft_model
                    .forward_speculative(prompt, self.draft_cache)
                {
                    return Some(Err(e));
                }
                ...
                self.state = SpeculativeState::Generate {
            ...
            SpeculativeState::Generate { last_token } => {
                ...
                let (_, target_tokens, target_logprobs) = match self.verify_draft_tokens(&input_seq)
                {
                    Ok(r) => r,
                    Err(e) => return Some(Err(e)),
                };
