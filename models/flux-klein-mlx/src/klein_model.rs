...
pub fn apply_rope(x: &Array, cos: &Array, sin: &Array) -> Result<Array, Exception> {
    ...
    // out0 = x0 * cos - x1 * sin
    // out1 = x1 * cos + x0 * sin
    let out0 = ops::subtract(
        &ops::multiply(&x0, &cos_val)?,
        &ops::multiply(&x1, &sin_val)?,
    )?;
    let out1 = ops::add(
        &ops::multiply(&x1, &cos_val)?,
        &ops::multiply(&x0, &sin_val)?,
    )?;
...
impl KleinDoubleBlock {
    pub fn new(hidden_size: i32, num_heads: i32, mlp_hidden: i32) -> Result<Self, Exception> {
        ...
        Ok(Self {
            ...
            // Pre-modulation normalization: LayerNorm without learnable affine (matches diffusers)
            img_norm1: LayerNormBuilder::new(hidden_size)
                .affine(false)
                .eps(1e-6)
                .build()?,
            img_norm2: LayerNormBuilder::new(hidden_size)
                .affine(false)
                .eps(1e-6)
                .build()?,
            txt_norm1: LayerNormBuilder::new(hidden_size)
                .affine(false)
                .eps(1e-6)
                .build()?,
            txt_norm2: LayerNormBuilder::new(hidden_size)
                .affine(false)
                .eps(1e-6)
                .build()?,
            ...
            // Image attention
            img_to_q: LinearBuilder::new(hidden_size, hidden_size)
                .bias(false)
                .build()?,
            img_to_k: LinearBuilder::new(hidden_size, hidden_size)
                .bias(false)
                .build()?,
            img_to_v: LinearBuilder::new(hidden_size, hidden_size)
                .bias(false)
                .build()?,
            img_norm_q: RmsNorm::new(head_dim)?,
            img_norm_k: RmsNorm::new(head_dim)?,
            img_to_out: LinearBuilder::new(hidden_size, hidden_size)
                .bias(false)
                .build()?,

            // Text attention
            txt_to_q: LinearBuilder::new(hidden_size, hidden_size)
                .bias(false)
                .build()?,
            txt_to_k: LinearBuilder::new(hidden_size, hidden_size)
                .bias(false)
                .build()?,
            txt_to_v: LinearBuilder::new(hidden_size, hidden_size)
                .bias(false)
                .build()?,
            txt_norm_q: RmsNorm::new(head_dim)?,
            txt_norm_k: RmsNorm::new(head_dim)?,
            txt_to_out: LinearBuilder::new(hidden_size, hidden_size)
                .bias(false)
                .build()?,

            // Image MLP
            img_mlp_in: LinearBuilder::new(hidden_size, mlp_hidden * 2)
                .bias(false)
                .build()?,
            img_mlp_out: LinearBuilder::new(mlp_hidden, hidden_size)
                .bias(false)
                .build()?,

            // Text MLP
            txt_mlp_in: LinearBuilder::new(hidden_size, mlp_hidden * 2)
                .bias(false)
                .build()?,
            txt_mlp_out: LinearBuilder::new(mlp_hidden, hidden_size)
                .bias(false)
                .build()?,
        })
    }
...
impl KleinSingleBlock {
    pub fn new(hidden_size: i32, num_heads: i32, mlp_hidden: i32) -> Result<Self, Exception> {
        ...
        Ok(Self {
            ...
            norm: LayerNormBuilder::new(hidden_size)
                .affine(false)
                .eps(1e-6)
                .build()?,
            to_qkv_mlp: LinearBuilder::new(hidden_size, fused_in)
                .bias(false)
                .build()?,
            to_out: LinearBuilder::new(fused_out, hidden_size)
                .bias(false)
                .build()?,
...
impl FluxKlein {
    pub fn new(params: FluxKleinParams) -> Result<Self, Exception> {
        ...
        // Input embeddings
        let x_embedder = LinearBuilder::new(params.in_channels, hidden)
            .bias(false)
            .build()?;
        let context_embedder = LinearBuilder::new(params.txt_embed_dim, hidden)
            .bias(false)
            .build()?;
        ...
        let proj_out = LinearBuilder::new(hidden, params.in_channels)
            .bias(false)
            .build()?;
