...
use mlx_rs::{
    ...
    error::Exception,
    module::{Module, Param},
    nn::{LayerNorm, LayerNormBuilder, Linear, QuantizedLinear, RmsNorm},
    ops,
...
use crate::klein_model::{
    apply_rope, compute_rope_freqs, FluxKlein, FluxKleinParams, KleinDoubleBlock, KleinSingleBlock,
    SharedModulation,
};
...
impl QuantizedFluxKlein {
    ...
    pub fn from_unquantized(
        ...
        println!(
            "  Quantizing transformer to INT{} (group_size={})...",
            bits, group_size
        );
        ...
        let single_blocks: Result<Vec<_>, _> = model
            ...
            .map(|(i, block)| {
                if i == 0 {
                    println!(
                        "    Quantizing {} single blocks...",
                        model.params.depth_single
                    );
                }
        ...
        Ok(Self {
            ...
            context_embedder: QuantizedLinear::try_from_linear(
            ...
            double_mod_img: QuantizedSharedModulation::from_unquantized(
                model.double_mod_img,
                group_size,
                bits,
            )?,
            double_mod_txt: QuantizedSharedModulation::from_unquantized(
                model.double_mod_txt,
                group_size,
                bits,
            )?,
            single_mod: QuantizedSharedModulation::from_unquantized(
                model.single_mod,
                group_size,
                bits,
            )?,
    ...
    pub fn compute_rope(txt_ids: &Array, img_ids: &Array) -> Result<(Array, Array), Exception> {
    ...
    pub fn forward_with_rope(
        ...
        for block in self.double_blocks.iter_mut() {
            let (new_img, new_txt) =
                block.forward(&img, &txt, &img_mod, &txt_mod, rope_cos, rope_sin)?;
            img = new_img;
            txt = new_txt;
        }
...
pub fn load_quantized_flux_klein(
    ...
    for i in 0..params.depth as usize {
        ...
        let block = QuantizedKleinDoubleBlock {
            ...
            img_norm1: LayerNormBuilder::new(hidden)
                .affine(false)
                .eps(1e-6)
                .build()?,
            img_norm2: LayerNormBuilder::new(hidden)
                .affine(false)
                .eps(1e-6)
                .build()?,
            txt_norm1: LayerNormBuilder::new(hidden)
                .affine(false)
                .eps(1e-6)
                .build()?,
            txt_norm2: LayerNormBuilder::new(hidden)
                .affine(false)
                .eps(1e-6)
                .build()?,
            ...
            // Image attention
            img_to_q: create_quantized_linear(
                &weights,
                &format!("{}.img_to_q", prefix),
                group_size,
                bits,
            )?,
            img_to_k: create_quantized_linear(
                &weights,
                &format!("{}.img_to_k", prefix),
                group_size,
                bits,
            )?,
            img_to_v: create_quantized_linear(
                &weights,
                &format!("{}.img_to_v", prefix),
                group_size,
                bits,
            )?,
            img_norm_q: load_rms_norm(
                &weights,
                &format!("{}.img_norm_q.weight", prefix),
                head_dim,
            )?,
            img_norm_k: load_rms_norm(
                &weights,
                &format!("{}.img_norm_k.weight", prefix),
                head_dim,
            )?,
            img_to_out: create_quantized_linear(
                &weights,
                &format!("{}.img_to_out", prefix),
                group_size,
                bits,
            )?,

            // Text attention
            txt_to_q: create_quantized_linear(
                &weights,
                &format!("{}.txt_to_q", prefix),
                group_size,
                bits,
            )?,
            txt_to_k: create_quantized_linear(
                &weights,
                &format!("{}.txt_to_k", prefix),
                group_size,
                bits,
            )?,
            txt_to_v: create_quantized_linear(
                &weights,
                &format!("{}.txt_to_v", prefix),
                group_size,
                bits,
            )?,
            txt_norm_q: load_rms_norm(
                &weights,
                &format!("{}.txt_norm_q.weight", prefix),
                head_dim,
            )?,
            txt_norm_k: load_rms_norm(
                &weights,
                &format!("{}.txt_norm_k.weight", prefix),
                head_dim,
            )?,
            txt_to_out: create_quantized_linear(
                &weights,
                &format!("{}.txt_to_out", prefix),
                group_size,
                bits,
            )?,

            // MLPs
            img_mlp_in: create_quantized_linear(
                &weights,
                &format!("{}.img_mlp_in", prefix),
                group_size,
                bits,
            )?,
            img_mlp_out: create_quantized_linear(
                &weights,
                &format!("{}.img_mlp_out", prefix),
                group_size,
                bits,
            )?,
            txt_mlp_in: create_quantized_linear(
                &weights,
                &format!("{}.txt_mlp_in", prefix),
                group_size,
                bits,
            )?,
            txt_mlp_out: create_quantized_linear(
                &weights,
                &format!("{}.txt_mlp_out", prefix),
                group_size,
                bits,
            )?,
        };
    ...
    for i in 0..params.depth_single as usize {
        ...
        let block = QuantizedKleinSingleBlock {
            ...
            norm: LayerNormBuilder::new(hidden)
                .affine(false)
                .eps(1e-6)
                .build()?,
            to_qkv_mlp: create_quantized_linear(
                &weights,
                &format!("{}.to_qkv_mlp", prefix),
                group_size,
                bits,
            )?,
            to_out: create_quantized_linear(
                &weights,
                &format!("{}.to_out", prefix),
                group_size,
                bits,
            )?,
...
pub fn quantize_and_save_flux_klein(
    ...
    println!(
        "Quantizing {} weights to {}bit (group_size={})...",
        weights.len(),
        bits,
        group_size
    );
    ...
    let is_norm_weight = |key: &str| -> bool {
        ...
        key.contains("_norm_q.")
            || key.contains("_norm_k.")
            || key.contains(".norm_q.")
            || key.contains(".norm_k.")
    };
