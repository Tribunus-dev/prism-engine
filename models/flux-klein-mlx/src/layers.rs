...
impl Modulation {
    ...
    pub fn new(dim: i32, is_double: bool) -> Result<Self, Exception> {
        ...
        let linear = LinearBuilder::new(dim, multiplier * dim)
            .bias(false)
            .build()?;
...
pub fn get_2d_rope_freqs(
    ...
    let h_freqs_exp = ops::broadcast_to(
        &h_freqs.reshape(&[height, 1, half_dim])?,
        &[height, width, half_dim],
    )?;
    let w_freqs_exp = ops::broadcast_to(
        &w_freqs.reshape(&[1, width, half_dim])?,
        &[height, width, half_dim],
    )?;
...
impl MlpEmbedder {
    ...
    pub fn new(in_dim: i32, hidden_dim: i32) -> Result<Self, Exception> {
        Ok(Self {
            ...
            linear2: LinearBuilder::new(hidden_dim, hidden_dim)
                .bias(false)
                .build()?,
        })
    }
...
impl SelfAttention {
    ...
    pub fn forward(&mut self, x: &Array, rope_freqs: Option<&Array>) -> Result<Array, Exception> {
        let shape = x.shape();
        let batch = shape[0];
        let seq_len = shape[1];
        ...
        let q = qkv
            .take_axis(&Array::from_slice(&[0i32], &[1]), 2)?
            .squeeze_axes(&[2])?;
        let k = qkv
            .take_axis(&Array::from_slice(&[1i32], &[1]), 2)?
            .squeeze_axes(&[2])?;
        let v = qkv
            .take_axis(&Array::from_slice(&[2i32], &[1]), 2)?
            .squeeze_axes(&[2])?;
...
impl Mlp {
    ...
    pub fn new(in_features: i32, hidden_features: i32) -> Result<Self, Exception> {
        ...
        Ok(Self {
            fc1: LinearBuilder::new(in_features, hidden_features * 2)
                .bias(false)
                .build()?,
            fc2: LinearBuilder::new(hidden_features, in_features)
                .bias(false)
                .build()?,
            hidden_features,
        })
...
impl DoubleStreamBlock {
    ...
    pub fn forward(
        ...
        let (img_shift1, img_scale1, img_gate1, img_shift2, img_scale2, img_gate2) = match img_mod {
            ModulationOutput::Double {
                shift1,
                scale1,
                gate1,
                shift2,
        ...
        let (txt_shift1, txt_scale1, txt_gate1, txt_shift2, txt_scale2, txt_gate2) = match txt_mod {
            ModulationOutput::Double {
                shift1,
                scale1,
                gate1,
...
impl SingleStreamBlock {
    ...
    pub fn new(hidden_size: i32, num_heads: i32, mlp_ratio: f32) -> Result<Self, Exception> {
        ...
        Ok(Self {
            ...
            linear1: LinearBuilder::new(hidden_size, linear1_out)
                .bias(false)
                .build()?,
            linear2: LinearBuilder::new(linear2_in, hidden_size)
                .bias(false)
                .build()?,
...
impl FinalLayer {
    ...
    pub fn new(hidden_size: i32, patch_size: i32, out_channels: i32) -> Result<Self, Exception> {
        ...
        Ok(Self {
            linear: LinearBuilder::new(hidden_size, output_dim)
                .bias(false)
                .build()?,
            ada_linear: LinearBuilder::new(hidden_size, hidden_size * 2)
                .bias(false)
                .build()?,
        })
    }
