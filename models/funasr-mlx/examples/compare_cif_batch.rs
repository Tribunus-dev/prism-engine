...
fn main() {
    ...
    println!(
        "Simulating encoder output: seq_len={}, hidden_dim={}\n",
        seq_len, hidden_dim
    );

    println!(
        "| Batch Size | Sequential | Batched   | Items/sec (Seq) | Items/sec (Batch) | Speedup |"
    );
    println!(
        "|------------|------------|-----------|-----------------|-------------------|---------|"
    );
    ...
    for &batch_size in &batch_sizes {
        ...
        for _ in 0..iterations {
            for b in 0..batch_size {
                ...
                let single_hidden: Vec<f32> =
                    hidden_data[offset_h..offset_h + seq_len * hidden_dim].to_vec();
                let single_alphas: Vec<f32> = alphas_data[offset_a..offset_a + seq_len].to_vec();

                // Simulate CIF fire for single item
                let _ =
                    simulate_cif_fire_single(&single_hidden, &single_alphas, seq_len, hidden_dim);
            }
        }
