...
fn main() {
    ...
    println!("| Duration | Frames | Manual DFT | FFT      | Speedup |");
    println!("|----------|--------|------------|----------|---------|");
    ...
    println!(
        "  Manual DFT: O(N²) = O({}) = ~{} operations/frame",
        n_fft * n_fft,
        n_fft * n_fft
    );
    let log_n = (n_fft as f32).log2().ceil() as usize;
    println!(
        "  FFT:        O(N log N) = O({} × {}) = ~{} operations/frame",
        n_fft,
        log_n,
        n_fft * log_n
    );
    println!(
        "  Theoretical speedup: ~{:.0}x",
        (n_fft * n_fft) as f32 / (n_fft * log_n) as f32
    );
