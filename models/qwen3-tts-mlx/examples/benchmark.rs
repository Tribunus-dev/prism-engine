...
impl BenchResult {
    fn print(&self) {
        ...
        eprintln!(
            "  Generation:    {:>8.1}ms  ({} frames, {:.1} fps)",
            self.generation_ms, self.generation_frames, gen_fps
        );
        eprintln!("  Decode:        {:>8.1}ms", self.decode_ms);
        eprintln!("  Total:         {:>8.1}ms", self.total_ms);
        eprintln!(
            "  Audio:         {:>8.2}s  ({} samples)",
            self.audio_duration_s, self.sample_count
        );
...
fn run_voice_clone_test(
    ...
    eprintln!("Running: {}", name);
    let (samples, timing) =
        synth.synthesize_voice_clone_with_timing(text, ref_samples, language, opts)?;
    let duration = samples.len() as f32 / synth.sample_rate as f32;
    let realtime = if timing.total_ms > 0.0 {
        duration as f64 / (timing.total_ms / 1000.0)
    } else {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!(
        "CustomVoice model loaded in {:.1}s\n",
    ...
    // Test 6: English with top-p sampling (exercises GPU top-p path)
    {
        ...
        results.push(run_test(
            &mut synth,
            "EN sampling top_p=0.9 top_k=50",
            en_text,
            &opts,
        )?);
    }
    ...
    if ref_audio_path.exists() {
        ...
        // Test 8: Voice clone, normal speed
        {
            ...
            results.push(run_voice_clone_test(
                &mut synth,
                "VoiceClone EN speed=1.0",
                en_text,
                &ref_samples,
                "english",
                &opts,
