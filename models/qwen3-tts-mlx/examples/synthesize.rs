...
fn main() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(ref ref_audio_path) = args.reference_audio {
        ...
        if args.reference_text.is_some() {
            eprintln!(
                "Warning: --reference-text ignored (ICL mode disabled). Using x-vector mode."
            );
        }
        ...
        let (samples, timing) = if args.speaker.is_some() {
            ...
            eprintln!(
                "Speaker+Instruct mode: speaker={}, instruct=\"{}\"",
                speaker, instruct
            );
            synth.synthesize_with_speaker_instruct_with_timing(&args.text, instruct, &opts)?
        } else {
