...
pub fn fused_snake_beta(
    ...
    if shape.len() < 2 {
        return Err(Exception::custom(
            "fused_snake_beta requires at least 2D input",
        ));
    }
    ...
    unsafe {
        ...
        if ret != 0 {
            ...
            return Err(Exception::custom(
                "fused_snake_beta Metal kernel execution failed",
            ));
        }
...
pub fn fused_residual_rmsnorm(
    ...
    if shape.len() < 2 {
        return Err(Exception::custom(
            "fused_residual_rmsnorm requires at least 2D input",
        ));
    }
    ...
    unsafe {
        ...
        if ret != 0 {
            ...
            mlx_sys::mlx_stream_free(stream);
            return Err(Exception::custom(
                "fused_residual_rmsnorm Metal kernel execution failed",
            ));
        }
