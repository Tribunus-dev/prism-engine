...
impl McpHandler for BrowserHandler {
    ...
    fn call(&self, request: ToolRequest<'_>, _context: &RequestContext, state: &DaemonState) -> Result<ToolResult> {
        ...
        let value = match self.name {
            ...
            "browser_find_element" => { let selector=request.args.get("selector").and_then(Value::as_str).ok_or_else(||anyhow!("selector is required"))?; browser.command("POST", "/element", Some(json!({"using":"css selector","value":selector})))? }
            "browser_get_tabs" => { let handles=browser.command("GET", "/window/handles", None)?.as_array().cloned().unwrap_or_default(); let current=browser.command("GET", "/window", None).ok(); let mut tabs=Vec::new(); for (index,handle) in handles.iter().enumerate() { browser.command("POST", "/window", Some(json!({"handle":handle})))?; let title=browser.command("GET", "/title", None)?.as_str().unwrap_or_default().to_string(); let url=browser.command("GET", "/url", None)?.as_str().unwrap_or_default().to_string(); tabs.push(json!({"title":title,"url":url,"index":index,"handle":handle})); } if let Some(handle)=current { let _=browser.command("POST", "/window", Some(json!({"handle":handle}))); } json!(tabs) }
