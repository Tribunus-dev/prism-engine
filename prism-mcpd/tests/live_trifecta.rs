...
impl Gate {
    fn new() -> Self {
        ...
        assert!(std::env::var("PRISM_MCPD_TEST_DUCKDB_PATH").is_ok(), "PRISM_MCPD_TEST_DUCKDB_PATH is required for live-trifecta");
        Self { state: tempfile::tempdir().unwrap(), artifacts: tempfile::tempdir().unwrap(), duckdb: tempfile::tempdir().unwrap(), postgres, valkey }
    }
    ...
    fn call(&self, name: &str, arguments: Value) -> Value {
        let mut child = Command::new(env!("CARGO_BIN_EXE_prism-mcpd"))
            ...
            .env("PRISM_MCPD_DUCKDB_PATH", self.duckdb.path().join("projection.duckdb"))
        ...
        assert!(output.status.success(), "MCP proxy failed: {}\n{}", String::from_utf8_lossy(&output.stderr), std::fs::read_to_string(self.state.path().join("daemon.log")).unwrap_or_default());
        String::from_utf8_lossy(&output.stdout).lines().filter_map(|line| serde_json::from_str::<Value>(line).ok()).find(|v| v["id"] == 2).expect("tool response")
    }

    fn result(&self, name: &str, args: Value) -> Value {
        let response = self.call(name, args);
        assert_eq!(response["result"]["isError"], false, "tool failed: {response}");
...
fn distributed_coordination_production_gate() {
    ...
    assert_eq!(gate.result("agent_session_start", json!({"session_id":&a,"agent_id":"a"}))["status"], "active");
    assert_eq!(gate.result("agent_session_start", json!({"session_id":&b,"agent_id":"b"}))["status"], "active");
    assert_eq!(gate.result("agent_work_create", json!({"work_id":&work,"work_title":"production gate"}))["status"], "queued");
    ...
    assert_eq!(gate.result("agent_path_lock", json!({"session_id":&a,"path":&path,"lock_kind":"write","ttl_seconds":60}))["acquired"], true);
    assert_eq!(gate.result("agent_path_lock", json!({"session_id":&b,"path":&path,"lock_kind":"write","ttl_seconds":60}))["acquired"], false);
    assert_eq!(gate.result("agent_work_handoff", json!({"work_id":&work,"from_session":&a,"to_session":&b,"context":{"gate":true}}))["ok"], true);
    let event = gate.result("agent_coordination_event", json!({"session_id":&a,"event_type":"production_gate","payload":{"ok":true}}));
    ...
    gate.result("agent_path_lock", json!({"session_id":&a,"path":&expired_path,"lock_kind":"write","ttl_seconds":1}));
    std::thread::sleep(std::time::Duration::from_secs(2));
    assert!(gate.result("agent_coordination_recover", json!({}))["expired_locks"].as_i64().unwrap() >= 1);
