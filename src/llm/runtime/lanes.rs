...
pub mod prism_backend {
    ...
    use super::super::super::manifest::{ExecutionLane, InferencePhase, QualificationStatus};
    ...
    impl ComputeLaneRouter {
        ...
        pub fn new() -> Self {
            ...
            let metal_runtime = ComputeLaneRuntime::new(backend, ComputeLaneId(0));
            Self {
        ...
        pub fn dispatch_metal_prefill(
            ...
            {
                let mut rt = self.metal_runtime.lock().map_err(|e| e.to_string())?;
        ...
        pub fn dispatch_accelerate(
            ...
            for op in &operations {
                match op.as_str() {
                    "softmax" | "rms_norm" | "add" | "mul" | "scale" | "matmul" | "sum" => {}
                    other => {
                        return Err(format!("unknown accelerate operation: {}", other));
                    }
                }
            }
            let receipt = AccelerateExecutionReceipt {
        ...
        pub fn dispatch_coreml_auxiliary(
            ...
            let receipt = CoreMlAuxiliaryReceipt {
                auxiliary_island_id: island_id.to_string(),
                artifact_digest: ArtifactDigest("coreml-auxiliary-compute".to_string()),
                source_epoch: dispatch.required_epoch,
                qualification_status: QualificationStatus::Accepted,
                input_contract_verified: true,
                output_contract_verified: true,
