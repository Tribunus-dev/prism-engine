...
pub mod cimage_engine {
    ...
    use mlx_rs::ops::{
        add, concatenate_device, multiply, quantize_device, quantized_matmul_device,
    };
    use mlx_rs::{fast, nn, Array, Stream};
    use std::path::Path;
    ...
    impl CimageEngine {
        pub fn load(path: &Path, graph: ModelGraph) -> Result<Self, String> {
            ...
            // Find norm weight
            let norm_key = all_keys
                .iter()
                .find(|k| k.contains("norm") && k.contains("weight") && !k.contains("layer"));
            ...
            let mut layers: Vec<LW> = (0..n_layers)
                .map(|_| LW {
                    q: None,
                    k: None,
                    v: None,
            ...
            for key in &all_keys {
                let li = match key
                    .split(".layers.")
                    .nth(1)
                    .and_then(|s| s.split('.').next().and_then(|n| n.parse::<usize>().ok()))
                {
            ...
            Ok(Self {
                ...
                head_dim,
                vocab_size: vs,
                rope_theta: cfg_rope(&graph),
                norm_eps: cfg_eps(&graph),
            })
        }
        ...
        pub fn generate(
            ...
            let t0 = Instant::now();
            let st = Stream::default();
            let mut kv: Vec<(Option<Array>, Option<Array>)> =
                (0..self.layers.len()).map(|_| (None, None)).collect();
            ...
            for _ in 0..max_tokens {
                for li in 0..self.layers.len() {
                    ...
                    let qr = fast::rope_device(
                        &q.reshape(&[1, nh, hd]).unwrap(),
                        hd,
                        false,
                    ...
                    .unwrap();
                    let kr = fast::rope_device(
                        &k.reshape(&[1, nkv, hd]).unwrap(),
                        hd,
                        false,
                        Some(self.rope_theta),
                        1.0,
                    ...
                    let attn = fast::scaled_dot_product_attention_device(
                        &qr,
                        fk_ref,
                        fv_ref,
                        1.0 / (hd as f32).sqrt(),
                ...
                let logits = match &self.lm_head {
                    ...
                    None => h
                        .slice(&[0, 0], &[1, self.vocab_size as i32], &[1, 1])
                        .unwrap()
                        .to_owned(),
                };
    ...
    fn embed(e: &Array, token: usize) -> Array {
        let s = e.shape();
        e.slice(&[token as i32, 0], &[token as i32 + 1, s[1]], &[1, 1])
            .unwrap()
            .to_owned()
    }
    ...
    fn load_one_qw_f32(r: &CImageReader, d: &[u8], key: &str) -> Option<Array> {
        ...
        for row in 0..om {
            ...
            for i in 0..16 {
                cb[i] = half::f16::from_bits(u16::from_le_bytes([
                    p[row * 32 + i * 2],
                    p[row * 32 + i * 2 + 1],
                ]))
                .to_f32();
    ...
    fn load_one_qw(r: &CImageReader, d: &[u8], key: &str, st: &Stream) -> Option<QW> {
        ...
        let (w, s, b) = quantize_device(
            &Array::from_slice(&f32v, &[om as i32, im as i32]),
            Some(64),
            Some(4),
            st,
        )
    ...
    fn cfg_attn(g: &ModelGraph) -> (usize, usize, usize) {
        for n in &g.nodes {
            if let ComputeNode::ScaledDotProductAttention {
                num_heads,
                num_kv_heads,
                head_dim,
            } = n
    ...
    fn cfg_rope(g: &ModelGraph) -> f32 {
        for n in &g.nodes {
            if let ComputeNode::RotaryEmbedding { rope_theta, .. } = n {
                return *rope_theta;
            }
        }
        10000.0
    }
    fn cfg_eps(g: &ModelGraph) -> f32 {
        for n in &g.nodes {
            if let ComputeNode::Norm { eps, .. } = n {
                return *eps;
            }
        }
        1e-6
